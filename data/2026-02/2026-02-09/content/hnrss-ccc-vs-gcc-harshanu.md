---
title: CCC vs GCC - Harshanu
url: https://harshanu.space/en/tech/ccc-vs-gcc/
site_name: hnrss
content_file: hnrss-ccc-vs-gcc-harshanu
fetched_at: '2026-02-09T11:22:46.183128'
original_url: https://harshanu.space/en/tech/ccc-vs-gcc/
author: Harshanu
date: '2026-02-09'
published_date: '2026-02-08T00:00:00+00:00'
...
Anthropic recently published a blog post aboutbuilding a C compiler entirely with Claude. They called it CCC (Claude’s C Compiler) and claimed it could compile the Linux kernel. 100% of the code was written by Claude Opus 4.6, a human only guided the process by writing test cases. That sounded interesting enough to test the claim and benchmark CCC against the industry standard GCC.

The source code of CCC is available atclaudes-c-compiler. It is written entirely in Rust, targeting x86-64, i686, AArch64 and RISC-V 64. The frontend, SSA-based IR, optimizer, code generator, peephole optimizers, assembler, linker and DWARF debug info generation are all implemented from scratch with zero compiler-specific dependencies. That is a lot of work for an AI to do.
...
1. Preprocessor: Handles#include,#defineand other directives. It takes the source code and produces expanded source code.
...
SQLite, on the other hand, is distributed as a single amalgamation file (one big.cfile). It is standard C, well-tested and self-contained. If your compiler can handle SQLite, it can handle a lot. If it cannot handle SQLite correctly, there is no point testing anything bigger.
...
2x Debian-based VMs on Proxmox hypervisor, each on a separate physical node, 6 vCPU, 16 GB RAM, 100 GB disk (NVMe)
...
Claude’s C Compiler, built from source with
--features gcc_m16

Kernel

Linux 6.9 (x86_64 defconfig)

SQLite

SQLite 3.46.0 amalgamation

Monitoring

/usr/bin/time -v
, custom system metrics logger (CPU%, RSS every 5s)

CCC was built with thegcc_m16Cargo feature, which delegates 16-bit real-mode boot code (-m16flag) to GCC. This is needed because CCC’s i686 backend produces code too large for the 32KB real-mode limit. The x86_64 C code is compiled entirely by CCC.

Accc_wrapper.shscript routes.Sassembly files to GCC (CCC does not process assembly) and all.cfiles to CCC.
...
4. CCC gets help where needed –gcc_m16feature for boot code, wrapper for assembly files
5. Same benchmark script –benchmark_sqlite.shruns identically on both VMs
...
42.5 min (couldn’t finish building binary)
...
42.5 min (just compilation of .c files)

User CPU Time

379.7 min

192.3 min (failed after .c file compilation)
...
1. __jump_tablerelocations – CCC generates incorrect relocation entries for kernel jump labels (used for static keys/tracepoints)
2. __ksymtabreferences – CCC produces malformed symbol table entries for kernel module exports
...
CCC-O0and-O2produce byte-identical binaries (4,374,024 bytes). CCC has 15 SSA optimization passes, but they all run at every optimization level. There is no tiered optimization – the-Oflag is accepted but completely ignored.
...
GCC’s-O2spends 7 minutes doing this work, and the payoff is clear: the resulting binary runs 1.7x faster (6.1s vs 10.3s).
...
CCC’s biggest performance problem is excessive register spilling. SQLite’s core execution enginesqlite3VdbeExecis a single function with 100+ local variables and a massive switch statement. CCC does not have good register allocation, so it spills almost all variables to the stack.
...
, %eax ; load loop counter

cmpl -36
(
%rbp
)
, %eax ; compare against n
...
, %rax ; load from deep stack offset

movq %rax, -0x2ae8
(
%rbp
)
 ; store to another deep stack offset
...
%rbp
 ...
 ; store to next offset

; ... dozens more memory-to-memory copies

CCC uses stack offsets up to -0x2ae8 (11,000 bytes deep) for a function with 32 variables. Every operation goes:stack -> rax -> stack, using%raxas a shuttle register.

#### Micro-benchmark: 32 Local Variables with Switch Statement

CCC: 8.604s
(
41.6x slower than GCC -O2
)
...
CCC is 4.2x slower than GCC O0 for register-heavy code. Insqlite3VdbeExecwith 100+ variables and 200+ switch cases, this ratio compounds to 100x+.
...
This means-O2provides zero benefit. Every binary CCC produces is effectively-O0quality, regardless of what flag you pass.
...
2. Linker compatibility: Generates incorrect relocations for kernel’s__jump_tableand__ksymtabsections
...
7. Compilation speed: Could only be compared with-O0as CCC does not do anything beyond this. CCC is around 25% slower vs GCC (87s vs 65s)
...
25% faster with GCC -O0 for SQLite

Binary size

GCC

CCC binaries are 2.7-3x larger

Runtime speed

GCC

CCC output is 737-158,000x slower

Memory (compiler)

GCC

CCC uses 2.3-5.9x more RAM
...
CCC ignores -O flags entirely

Kernel linking

GCC

CCC fails at link stage

## The Hello World That Could Not

Within hours of Anthropic releasing CCC, someone openedissue #1– “Hello world does not compile”. The example straight from the README did not work on a fresh Fedora or Ubuntu install:
...
Meanwhile, GCC compiled it just fine. The issue was that CCC’s preprocessor did not search the right system include paths forstddef.handstdarg.h(these come from the compiler, not the C library). It got 288 thumbs-up reactions, over 200 comments and turned into one of those legendary GitHub threads where people tag@claudeasking it to fix the bug, ask@grokfor summaries and post comments like “my job is safe”.
...
3. Optimization flags are decorative. Passing-O2or-O3to CCC does literally nothing – the output binary is byte-identical to-O0.
...
* Debian/Ubuntu withbuild-essential,cargo,git,flex,bison,bc,libelf-dev,libssl-dev
...
All scripts, results and graphs are available atcompare-claude-compiler.
...
We share photos, videos, and stories from our travels, hikes, and cycling trips. Follow us to see where we're going next!
