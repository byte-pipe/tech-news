...
The developer experience was excellent as well. Deploying a new version of your CGI program was just a matter of copying it to thecgi-bin/directory on your web server.

## Hug of death#

Typical web servers of this time had 1-2 CPUs and 1-4 GB of memory.

Most web servers ran Apache, which would fork anhttpdprocess for every connection, each of which took a significant amount of memory. This would limit the maximum concurrency to fewer than 100 connections in most cases.

This made it incredibly easy toSlashdota website just by linking to it from a popular site.
...
To run a benchmark of CGI on a modern system, I created a little CGI program and ran it under Apache and (just for fun) a custom Gonet/httpserver. I usedplowto make concurrent HTTP requests and measure the results.
...
I quickly wrote a littleguestbookprogram that could be used to allow visitors to leave comments at the bottom of a website.
...
Sprintf
(
"file:%s?_journal_mode=WAL&_synchronous=NORMAL&_busy_timeout=5000&_cache_size=10000"
,
databaseFile
)
...
\
 ...
 --body
"name=John+Carmack&message=Hello+from+id+software%21"
...
Benchmarking http://localhost:1111/~jakegold/cgi-bin/guestbook.cgi with
100000
 request
(
s
...
@ Real-time charts is listening on http://
[
::
]
:18888
