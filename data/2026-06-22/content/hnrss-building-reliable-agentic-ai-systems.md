...
site_name: hnrss
content_file: hnrss-building-reliable-agentic-ai-systems
fetched_at: '2026-06-22T00:53:16.962448'
original_url: https://martinfowler.com/articles/reliable-llm-bayer.html
date: '2026-06-21'
description: Building reliable agentic AI systems
tags:
- hackernews
- hnrss
---
...
Sarang Kulkarni is a Principal Consultant at Thoughtworks, working at the intersection of
 ...
 anO’Reilly
 courseon building production-ready RAG applications.
...
* The Agentic RAG SystemClarify User IntentThink & Plan: Process ReflectionThe Researcher AgentThe Reflection Agent: Data Validation and SufficiencyThe Writer Agent: Answer Synthesis and Formatting
* Clarify User Intent
...
* Building Trust in a Production LLM SystemTransparency and ExplainabilityEvaluationMonitoring
...
While this post focuses on the technical architecture and engineering challenges, our paper
 published inFrontiers in Artificial Intelligencecovers the
...
The system functions as an interactive conversational UI, powered by a robust backend
 ...
 accurate, context-rich answers, is orchestrated usingLangGraphand served via aFastAPIapplication.

Figure 1provides the system context—UI, backend, data
 stores, LLM fallbacks, and observability—whileFigure 2zooms into how the system coordinates its specialized agents.
...
* Vector representations of all study reports are stored inOpenSearch, forming
 the coreknowledge basefor information retrieval.
* Curatedstructured data, resulting from various ETL and harmonization
 processes, is accessed viaAthena.
* Thestateof the agent's execution is meticulously tracked. After each logical
 step (a LangGraph node execution), the corresponding state is persisted inPostgreSQLusing aLangGraph checkpointer.
* Broaderapplication-level stateis managed inDynamoDB.
...
* Observability and Evaluation: the entire system is monitored for performance and
 reliability:
* General system health and metrics are tracked usingCloudwatch.
* Langfuseserves as the primary observability tool, providing detailed traces of
...
PRINCE incorporates an agenticRAGsystem (Figure 2) to handle complex user requests that require multiple
 steps, reasoning, and interaction with different tools or data sources. This setup,
 implemented usingLangGraph, orchestrates the overall workflow and leveragesResearcher
 Agent,Writer Agent, andReflection Agentfor specific tasks. The system
...
TheClarify User Intentstep serves as the first line of defense against
...
This ensures the system enhances the query with the necessary constraints to target the
 correct tools. We are also optimizing this by developingdomain-level selectionin
...
TheThink & Planstep is responsible for devising a strategy to fulfill the
 user's request. This critical component gives the system a dedicated space to reason about
 the next steps before taking action—a technique inspired byAnthropic's Think tool.
 Importantly, this step performsprocess reflection: evaluating whether the agent is
...
In multi-step agentic workflows, particularly those involving many sequential actions,
 ...
 trajectory leading toward the user's goal? TheThink & Planstep provides this
...
Beyond tool selection, theThink & Planstep is essential for orchestrating
...
TheResearcher Agentserves as the system's primary information gatherer. As we
 onboard new scientific domains onto PRINCE, we consistently observe that data falls into
 two primary categories:structuredandunstructured. While specific
...
To effectively harvest insights from this diverse landscape, the Researcher Agent employs
 ahybrid retrieverapproach focused on two distinct
 patterns:
...
Each chunk is enriched with study‑ and section‑level metadata from Amazon Athena (for
 ...
 embedded and indexed inAmazon OpenSearch Service,
...
* Reranking: the initial set of retrieved chunks (k=~20) is then refined using aRerankstep. A cross-encoder model (bge-reranker-large)
...
While RAG excels at unstructured data, queries requiring precise filtering,
 ...
 numerical assay results including dosage groups. As shown in theResearcher Agentcan intelligently decide to hand over such queries to the
 Text-to-SQL tool.
...
While theThink & Planstep provides process reflection, theReflection
 Agentperforms a complementary but distinct type of reflection:data reflection.
...
As illustrated in the research workflow diagram (Figure 2), after initial information retrieval and 'think
 & plan' loops, theReflection Agentis invoked when Think & Plan step
 ...
 to provide a complete response, theReflection Agentgenerates specific follow-up
 questions designed to acquire the necessary missing information. These follow-up questions
 are then handed back to theThink & Planstep, which initiates further
 retrieval steps to obtain more comprehensive results. This iterative process of data
 validation and subsequent information retrieval, driven by theReflection Agent's
...
Once the Researcher Agent has collected the relevant evidence from RAG and Text-to-SQL,
 theWriter Agentis responsible for turning that raw material into the final answer
...
For more complex responses—such as multi-section summaries or partially filled regulatory
 ...
 specific parts. This design enables a lightweight form of reflection focused onanswer
 completeness and
 presentation, complementing the Reflection Agent's focus ondata sufficiencyearlier in the workflow. Importantly, all outputs from these regulatory drafting workflows
...
* Dataset Evaluations: conducted whenever significant changes are made to the core
 ...
 generated response against the reference answer, yielding quantitative metrics such asFaithfulness(degree to which the answer is supported by context),Answer
...
* State Persistence: the state of the entire workflow graph is persistently stored,
 ...
 storing theAgent State, representing the progress of the agents through the
