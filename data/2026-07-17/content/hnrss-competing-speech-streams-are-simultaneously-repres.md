---
title: Competing speech streams are simultaneously represented in the human cortex during attention switching | PLOS Biology
url: https://journals.plos.org/plosbiology/article?id=10.1371/journal.pbio.3003876
site_name: hnrss
content_file: hnrss-competing-speech-streams-are-simultaneously-repres
fetched_at: '2026-07-17T19:28:28.438683'
original_url: https://journals.plos.org/plosbiology/article?id=10.1371/journal.pbio.3003876
date: '2026-07-17'
description: How the brain rapidly switches attention between competing speech streams in complex listening environments is not fully understood. This study shows that neural tracking of a new speaker emerges before disengagement, with transient dual encoding and reduced alpha power supporting flexible auditory attention.
tags:
- hackernews
- hnrss
---
...
Copyright:© 2026 Carta et al. This is an open access article distributed under the terms of theCreative Commons Attribution License, which permits unrestricted use, distribution, and reproduction in any medium, provided the original author and source are credited.
...
* TIFForiginal image
Fig 1. 
 Experiment overview and validation.
...
* TIFForiginal image
Fig 3. 
 Comparing the start and end transition points for the disengagement and engagement processes after the attentions switching cue.
...
* TIFForiginal image
Fig 4. 
 Investigating lexical prediction mechanisms during attention switching.
...
When considering the dynamic nature of the attention switching task, the definition of what context to include for the current word prediction becomes a non-trivial problem. From the machine perspective, given a wordwbelonging to, e.g., the left stream, the LLM would predict it more easily when provided with all the available linguistic context from the left stream. However, from the neural/behavioral perspective, since participants were flexibly re-orienting their attention between left and right streams, the optimal context would be impacted by the attention switch and, potentially, store information of previously attended blocks. To compare these context-accumulation alternatives, we represented context according to four alternative representations. A machine-ideal model, Oracle, considers as context all words preceding the current word in one particular stream, whether they were attended or unattended. As such, this model isunawareof the switch of attention. Among more neurally-plausible and switch-aware context accumulation models, we constructed the Attention model, which incorporates as context any previously attended block from both left and right speech streams, and the Speaker Specific model, which displays a speech stream bias, whereby only previously attended blocks of the same stream are included as context for lexical prediction of the current block. Similarly to Attention and Speaker Specific, the fourth model, Reset, is switch-aware, but not context-aware. In fact, it does not keep track of any previous speech block, neither attended nor unattended, and instead resets the context following each attention switch cue.

### Temporal Response Function (TRF) and analysis procedure

In this study, we quantified neural tracking of speech features as the ability to predict unseen EEG data (encoding) or reconstruct unseen stimulus features (decoding), with a linear model based on lagged linear regression, the TRF. A TRF model was fit for each participant separately, using a leave-one-trial-out cross-validation procedure to avoid overfitting, and considering a range of time lags from −100 to 600 ms in the case of the encoding models, and between 0 and 600 ms for the decoding models. The TRF model calculation applied Tikhonov regularization, with the regularization parameter (λ) tuned via an exhaustive search across a logarithmic scale from 10−6to 104. This search was conducted on the training fold during each iteration of the cross-validation process, and the optimalλwas selected as the parameter that yielded the highest correlation (Pearson’sr) between true and predicted/reconstructed data.

We first constructed an Attended speech model from the combination of attended segments of left and right speech streams. We then fitted TRF models by coupling the EEG and the Attended stimulus features. With a decoding approach, we reconstructed the envelope of each trial based on the Attended TRF model and then correlated it with the true envelopes of left and right streams for the corresponding trial. Since the left or right true attention label was known throughout the trial, classification accuracy could be computed as the percentage of left or right segments correctly classified as attended (Fig 1C). We also computed the null distribution of the classification accuracy by randomly shuffling the left and right labels for 100 iterations. We employed a range of decoding window lengths (1,2,4,8,16,32 s) to assess the impact of correlation window size on the model’s ability to correctly classify the attended stimulus.

To observe the evolution of neural tracking of left and right streams over time with a neurophysiologically interpretable approach, we calculated an encoding TRF model where true and predicted EEG signals were correlated using a sliding window approach (MATLAB’smovcorrfunction) with a step size of 0.125 s. Accordingly, the estimated temporal dynamics are window-dependent and are interpreted as relative, rather than absolute, measures of neural timing. The encoding TRFs were fit with a multivariate set of attended stimulus features including envelope (Env), word onset (WO) and word surprisal (WS) or word entropy (WE), to assess the joint impact of both acoustic and semantic features on the temporal dynamics of the attention switch.

The re-orienting of attention around the switch can be analyzed in terms of disengaging stream (attended > unattended) and engaging (unattended > attended) stream. To compare the onset and offset points of these processes, we first selected participants depending on their attentional bias over the course of the switch. Based on the reference 4-s encoding window, we determined the single-subject classification accuracy in a 15 s window before and after the attention switching cue. We then found those participants displaying a lower-than-chance (50%) classification performance either in the pre- or post-switch window and excluded them from further analyses on the switching temporal dynamics. Start and end points of disengagement and engagement processes were identified for each subject by fitting a piecewise linear regression model to the EEG prediction correlation values for the disengaging and the engaging streams, around the attention switch cue. For each of the EEG prediction correlation streams, the model incorporates the average pre-switch and post-switch values as constants and tries to estimate the breakpoints at which these flat segments connect, i.e., the onset and offset points of the process. An initial guess for the breakpoints was provided based on reasonable assumptions (e.g., that the start of the disengagement/engagement could not occur before the switching cue), and the fitting was done with MATLAB’slsqcurvefitfunction, which minimizes the difference between the model and the observed data.

When computing encoding TRFs comparing different kinds of context-accumulation models, we adopted an approach optimized for highlighting the effects of lexical information processing [83]. With this method, the prediction correlation between true and predicted EEG data is computed within a window covering the range from 300 to 500 ms after word onset, allowing to effectively enhance the impact of semantic encoding.

### Event-related spectral perturbation (ERSP)

Previous studies have related decreased parietal alpha-band (8–12 Hz) EEG power to increased listening effort and have shown that more listening effort is required to switch attention than to sustain attention on a particular speaker [29]. Since we were interested in assessing the time course of alpha power and how it would compare to the temporal dynamics of the TRF prediction correlations, we computed event-related spectral perturbation (ERSP) in the alpha frequency band as a proxy of effort around the switch. Since neural activity in the alpha frequency band is sensitive to ocular artifacts, we performed Independent Component Analysis (ICA) using standard EEGLAB functions [84]. For each subject, we identified independent components reflecting ocular artifacts (such as blinks and horizontal eye movements) based on their temporal characteristics and spatial topographies, and we removed them from the data. ICA-based artifact removal was applied only for the alpha-band ERSP analysis and not for neural tracking. TRF estimates are based on stimulus–response covariance over extended time lags and are therefore unlikely to be influenced by brief eye movements around the switch, which do not systematically covary with the speech signal. TRFs were fit on the full dataset, and inspection of the TRF weights did not reveal near-zero latency spikes indexing ocular contamination. Although eye movements could in principle affect the SNR around the switch, we did not observe corresponding changes in EEG prediction correlations (e.g.,Fig 4B, Oracle model), and switch direction was counterbalanced across left and right streams.

For each participant, trial, and attention switch, ERSP was computed as the relative change in power spectral density (PSD) compared to a baseline. We first defined a segment (±15 s) around the switching cue and, to estimate ERSP with the same temporal resolution used for the neural tracking analysis, we chose a sliding window of 4 s, with a step size of 0.125 s. This analysis was also replicated across a range of sliding window lengths (1, 2, 8 s), in line with the neural tracking analysis. Along the switching segment, PSD was calculated at each forward step within the sliding window via Welch’s method (MATLAB’spwelchfunction) and summed across the alpha band frequency bins (8–12 Hz). The PSD computed at each step was compared to the reference PSD value of a baseline, consisting of the first sliding window in the pre-switch segment, producing a series of ERSP values (in dB) that reflects the relative change in alpha power during the switch.

### Statistical analysis

Statistical analyses were performed using repeated measures ANOVAs with two or three factors, withF-values reported asF(dftime, dferror). When the assumption of sphericity wasn’t met, as revealed by Mauchly’s test of sphericity, a correction was applied with the Greenhouse–Geisser’s method. When a significant main effect was found, post-hoc tests were run, andp-values were corrected for multiple comparisons with the Holm method. To compare the significance of a time series against zero, a Wilcoxon signed rank test was used for each time point, and the false discovery rate (FDR) was controlled with the Benjamini and Hochberg procedure. We report FDR-adjustedp-values. Descriptive statistics for the neurophysiology results are reported as a combination of mean and standard error (SE).

## Supporting information

### S1 Fig.Time latencies of the encoding switch of EEG prediction correlations (turquoise) and alpha ERSP minimum (red) for four sliding window lengths.

Bars represent the average time latencies across participants and electrodes; error bars indicate the SEM across participants. Stars represent statistically significant comparisons (Significance levels: *p< 0.05, **p< 0.01, ***p< 0.001). Data supporting this figure is available at:https://zenodo.org/records/20569817.

https://doi.org/10.1371/journal.pbio.3003876.s001

(TIF)

### S2 Fig.Neural tracking time courses and scalp topographies for Speaker 1 and Speaker 2 across the attention-switch cue.

EEG prediction correlations result from a model including Envelope, Word Onset, and Word Surprisal features using a sliding window of 4s. Scalp topographies indicate the predictive performance of the TRF model across electrodes at selected time points (black dots), for the engagement and disengagement processes. Coloured horizontal bars indicate the instructed focus of attention before and after the switching cue. The turquoise dot marks the encoding switch, when EEG tracking shifts from Spk1 to Spk2. Overlaid piecewise linear fits represent disengagement from the initially attended speaker and engagement with the newly attended speaker. Blue and yellow hexagrams denote the onset of disengagement and engagement, respectively, while diamonds indicate the end of each transition. Fits are shown on grand-average tracking curves for illustration; participant-level estimates are reported inFig 2C. Data supporting this figure is available at:https://zenodo.org/records/20569817.

https://doi.org/10.1371/journal.pbio.3003876.s002

(TIF)

## Acknowledgments

We thank ADAPT, the Research Ireland Centre for AI-Driven Digital Content Technology, and the Eriksholm Research Centre, part of Oticon, for the support. We thank Amirhossein Chalehchaleh, Emily Ip, and Martin Winchester for their help with data collection, and Dr. Enda Bates for assistance in facilitating access to the experimental space. For the purpose of Open Access, the author has applied a CC BY public copyright licence to any Author Accepted Manuscript version arising from this submission.

## References

1. 1.Power AJ, Foxe JJ, Forde E-J, Reilly RB, Lalor EC. At what time is the cocktail party? A late locus of selective attention to natural speech. Eur J Neurosci. 2012;35(9):1497–503. pmid:22462504* View Article
* PubMed/NCBI
* Google Scholar
2. 2.Zion Golumbic EM, Ding N, Bickel S, Lakatos P, Schevon CA, McKhann GM, et al. Mechanisms underlying selective neuronal tracking of attended speech at a “cocktail party”. Neuron. 2013;77(5):980–91. pmid:23473326* View Article
* PubMed/NCBI
* Google Scholar
3. 3.Puvvada KC, Simon JZ. Cortical representations of speech in a multitalker auditory scene. J Neurosci. 2017;37(38):9189–96. pmid:28821680* View Article
* PubMed/NCBI
* Google Scholar
4. 4.Cherry EC. Some experiments on the recognition of speech, with one and with two ears. J Acoust Soc Am. 1953;25(5):975–9.* View Article
* Google Scholar
5. 5.Bronkhorst AW. The cocktail-party problem revisited: early processing and selection of multi-talker speech. Atten Percept Psychophys. 2015;77(5):1465–87. pmid:25828463* View Article
* PubMed/NCBI
* Google Scholar
6. 6.Rivenez M, Darwin CJ, Guillaume A. Processing unattended speech. J Acoust Soc Am. 2006;119(6):4027–40. pmid:16838545* View Article
* PubMed/NCBI
* Google Scholar
7. 7.Har-Shai Yahav P, Zion Golumbic E. Linguistic processing of task-irrelevant speech at a cocktail party. Elife. 2021;10:e65096. pmid:33942722* View Article
* PubMed/NCBI
* Google Scholar
8. 8.Kaufman M, Zion Golumbic E. Listening to two speakers: capacity and tradeoffs in neural speech tracking during Selective and Distributed Attention. Neuroimage. 2023;270:119984. pmid:36854352* View Article
* PubMed/NCBI
* Google Scholar
9. 9.O’Sullivan JA, Power AJ, Mesgarani N, Rajaram S, Foxe JJ, Shinn-Cunningham BG, et al. Attentional selection in a cocktail party environment can be decoded from single-trial EEG. Cereb Cortex. 2015;25(7):1697–706. pmid:24429136* View Article
* PubMed/NCBI
* Google Scholar
10. 10.Mesgarani N, Chang EF. Selective cortical representation of attended speaker in multi-talker speech perception. Nature. 2012;485:233–6.* View Article
* Google Scholar
11. 11.Simon JZ. The encoding of auditory objects in auditory cortex: insights from magnetoencephalography. Int J Psychophysiol. 2015;95(2):184–90. pmid:24841996* View Article
* PubMed/NCBI
* Google Scholar
12. 12.Evans S, McGettigan C, Agnew ZK, Rosen S, Scott SK. Getting the cocktail party started: masking effects in speech perception. J Cogn Neurosci. 2016;28(3):483–500. pmid:26696297* View Article
* PubMed/NCBI
* Google Scholar
13. 13.Hausfeld L, Hamers IMH, Formisano E. FMRI speech tracking in primary and non-primary auditory cortex while listening to noisy scenes. Commun Biol. 2024;7(1):1217. pmid:39349723* View Article
* PubMed/NCBI
* Google Scholar
14. 14.O’Sullivan J, Herrero J, Smith E, Schevon C, McKhann GM, Sheth SA, et al. Hierarchical encoding of attended auditory objects in multi-talker speech perception. Neuron. 2019;104(6):1195-1209.e3. pmid:31648900* View Article
* PubMed/NCBI
* Google Scholar
15. 15.Teoh ES, Ahmed F, Lalor EC. Attention differentially affects acoustic and phonetic feature encoding in a multispeaker environment. J Neurosci. 2022;42(4):682–91. pmid:34893546* View Article
* PubMed/NCBI
* Google Scholar
16. 16.Park H, Gross J. Get the gist of the story: neural map of topic keywords in multi-speaker environment. eLife Sciences Publications, Ltd. 2023.* View Article
* Google Scholar
17. 17.Ding N, Simon JZ. Neural coding of continuous speech in auditory cortex during monaural and dichotic listening. J Neurophysiol. 2012;107(1):78–89. pmid:21975452* View Article
* PubMed/NCBI
* Google Scholar
18. 18.Egner T. Principles of cognitive control over task focus and task switching. Nat Rev Psychol. 2023;2(11):702–14. pmid:39301103* View Article
* PubMed/NCBI
* Google Scholar
19. 19.Luo T, Xu M, Zheng Z, Okazawa G. Limitation of switching sensory information flow in flexible perceptual decision making. Nat Commun. 2025;16(1):172. pmid:39747100* View Article
* PubMed/NCBI
* Google Scholar
20. 20.Ravizza SM, Carter CS. Shifting set about task switching: behavioral and neural evidence for distinct forms of cognitive flexibility. Neuropsychologia. 2008;46(12):2924–35. pmid:18601941* View Article
* PubMed/NCBI
* Google Scholar
21. 21.Wager TD, Jonides J, Reading S. Neuroimaging studies of shifting attention: a meta-analysis. Neuroimage. 2004;22(4):1679–93. pmid:15275924* View Article
* PubMed/NCBI
* Google Scholar
22. 22.Rogers RD, Monsell S. Costs of a predictible switch between simple cognitive tasks. J Exp Psychol General. 1995;124(2):207–31.* View Article
* Google Scholar
23. 23.Lawo V, Fels J, Oberem J, Koch I. Intentional attention switching in dichotic listening: exploring the efficiency of nonspatial and spatial selection. Q J Exp Psychol (Hove). 2014;67(10):2010–24. pmid:25248101* View Article
* PubMed/NCBI
* Google Scholar
24. 24.Oberem J, Koch I, Fels J. Intentional switching in auditory selective attention: Exploring age-related effects in a spatial setup requiring speech perception. Acta Psychol (Amst). 2017;177:36–43. pmid:28456098* View Article
* PubMed/NCBI
* Google Scholar
25. 25.Meister H, Wenzel F, Gehlen AK, Kessler J, Walger M. Static and dynamic cocktail party listening in younger and older adults. Hear Res. 2020;395:108020. pmid:32698114* View Article
* PubMed/NCBI
* Google Scholar
26. 26.Miran S, Akram S, Sheikhattar A, Simon JZ, Zhang T, Babadi B. Real-time tracking of selective auditory attention from M/EEG: a bayesian filtering approach. Front Neurosci. 2018;12:262. pmid:29765298* View Article
* PubMed/NCBI
* Google Scholar
27. 27.Akram S, Presacco A, Simon JZ, Shamma SA, Babadi B. Robust decoding of selective auditory attention from MEG in a competing-speaker environment via state-space modeling. Neuroimage. 2016;124(Pt A):906–17. pmid:26436490* View Article
* PubMed/NCBI
* Google Scholar
28. 28.Teoh ES, Lalor EC. EEG decoding of the target speaker in a cocktail party scenario: considerations regarding dynamic switching of talker location. J Neural Eng. 2019;16(3):036017. pmid:30836345* View Article
* PubMed/NCBI
* Google Scholar
29. 29.Haro S, Rao HM, Quatieri TF, Smalt CJ. EEG alpha and pupil diameter reflect endogenous auditory attention switching and listening effort. Eur J Neurosci. 2022;55(5):1262–77. pmid:35098604* View Article
* PubMed/NCBI
* Google Scholar
30. 30.Agmon G, Yahav PH-S, Ben-Shachar M, Golumbic EZ. Attention to speech: mapping distributed and selective attention systems. Cereb Cortex. 2022;32(17):3763–76. pmid:34875678* View Article
* PubMed/NCBI
* Google Scholar
31. 31.de Cheveigné A, Wong DDE, Di Liberto GM, Hjortkjær J, Slaney M, Lalor E. Decoding the auditory brain with canonical component analysis. Neuroimage. 2018;172:206–16. pmid:29378317* View Article
* PubMed/NCBI
* Google Scholar
32. 32.Heilbron M, Armeni K, Schoffelen J-M, Hagoort P, de Lange FP. A hierarchy of linguistic predictions during natural language comprehension. Proc Natl Acad Sci U S A. 2022;119(32):e2201968119. pmid:35921434* View Article
* PubMed/NCBI
* Google Scholar
33. 33.Broderick MP, Anderson AJ, Lalor EC. Semantic context enhances the early auditory encoding of natural speech. J Neurosci. 2019;39(38):7564–75. pmid:31371424* View Article
* PubMed/NCBI
* Google Scholar
34. 34.O’Sullivan J, Chen Z, Herrero J, McKhann GM, Sheth SA, Mehta AD, et al. Neural decoding of attentional selection in multi-speaker environments without access to clean sources. J Neural Eng. 2017;14(5):056001. pmid:28776506* View Article
* PubMed/NCBI
* Google Scholar
35. 35.Choudhari V, Han C, Bickel S, Mehta AD, Schevon C, McKhann GM, et al. Brain-controlled augmented hearing for spatially moving conversations in multi-talker environments. Adv Sci (Weinh). 2024;11(41):e2401379. pmid:39248654* View Article
* PubMed/NCBI
* Google Scholar
36. 36.Alickovic E, Lunner T, Gustafsson F, Ljung L. A tutorial on auditory attention identification methods. Front Neurosci. 2019;13:153. pmid:30941002* View Article
* PubMed/NCBI
* Google Scholar
37. 37.Gillis M, Vanthornhout J, Simon JZ, Francart T, Brodbeck C. Neural markers of speech comprehension: measuring EEG tracking of linguistic speech representations, controlling the speech acoustics. J Neurosci. 2021;41(50):10316–29. pmid:34732519* View Article
* PubMed/NCBI
* Google Scholar
38. 38.Alickovic E, Ng EHN, Fiedler L, Santurette S, Innes-Brown H, Graversen C. Effects of hearing aid noise reduction on early and late cortical representations of competing talkers in noise. Front Neurosci. 2021;15:636060. pmid:33841081* View Article
* PubMed/NCBI
* Google Scholar
39. 39.Raghavan VS, O’Sullivan J, Herrero J, Bickel S, Mehta AD, Mesgarani N. Improving auditory attention decoding by classifying intracranial responses to glimpsed and masked acoustic events. Imaging Neurosci (Camb). 2024;2:imag–2–00148. pmid:39867597* View Article
* PubMed/NCBI
* Google Scholar
40. 40.Hjortkjær J, Wong DDE, Catania A, Märcher-Rørsted J, Ceolini E, Fuglsang SA, et al. Real-time control of a hearing instrument with EEG-based attention decoding. J Neural Eng. 2025;22(1):10.1088/1741-2552/ad867c. pmid:39996608* View Article
* PubMed/NCBI
* Google Scholar
41. 41.Geirnaert S, Vandecappelle S, Alickovic E, de Cheveigne A, Lalor E, Meyer BT, et al. Electroencephalography-based auditory attention decoding: toward neurosteered hearing devices. IEEE Signal Process Mag. 2021;38(4):89–102.* View Article
* Google Scholar
42. 42.Wilroth J, Alickovic E, Skoglund MA, Signoret C, Rönnberg J, Enqvist M. Improving tracking of selective attention in hearing aid users: the role of noise reduction and nonlinearity compensation. eNeuro. 2025;12(2):ENEURO.0275-24.2025. pmid:39880674* View Article
* PubMed/NCBI
* Google Scholar
43. 43.Katthi JR, Ganapathy S, Kothinti S, Slaney M. Deep canonical correlation analysis for decoding the auditory brain. Annu Int Conf IEEE Eng Med Biol Soc. 2020;2020:3505–8. pmid:33018759* View Article
* PubMed/NCBI
* Google Scholar
44. 44.Fuglsang SA, Dau T, Hjortkjær J. Noise-robust cortical tracking of attended speech in real-world acoustic scenes. Neuroimage. 2017;156:435–44. pmid:28412441* View Article
* PubMed/NCBI
* Google Scholar
45. 45.Shinn-Cunningham BG. Object-based auditory and visual attention. Trends Cogn Sci. 2008;12(5):182–6. pmid:18396091* View Article
* PubMed/NCBI
* Google Scholar
46. 46.Bregman AS. Auditory scene analysis: the perceptual organization of sound. The MIT Press; 1990.* View Article
* Google Scholar
47. 47.Corbetta M, Shulman GL. Control of goal-directed and stimulus-driven attention in the brain. Nat Rev Neurosci. 2002;3(3):201–15. pmid:11994752* View Article
* PubMed/NCBI
* Google Scholar
48. 48.Corbetta M, Patel G, Shulman GL. The reorienting system of the human brain: from environment to theory of mind. Neuron. 2008;58(3):306–24. pmid:18466742* View Article
* PubMed/NCBI
* Google Scholar
49. 49.Ding N, Simon JZ. Emergence of neural encoding of auditory objects while listening to competing speakers. Proc Natl Acad Sci U S A. 2012;109(29):11854–9. pmid:22753470* View Article
* PubMed/NCBI
* Google Scholar
50. 50.Hausfeld L, Riecke L, Valente G, Formisano E. Cortical tracking of multiple streams outside the focus of attention in naturalistic auditory scenes. Neuroimage. 2018;181:617–26. pmid:30048749* View Article
* PubMed/NCBI
* Google Scholar
51. 51.Broderick MP, Anderson AJ, Di Liberto GM, Crosse MJ, Lalor EC. Electrophysiological correlates of semantic dissimilarity reflect the comprehension of natural, narrative speech. Curr Biol. 2018;28(5):803-809.e3. pmid:29478856* View Article
* PubMed/NCBI
* Google Scholar
52. 52.Anderson AJ, Davis C, Lalor EC. Deep-learning models reveal how context and listener attention shape electrophysiological correlates of speech-to-language transformation. PLoS Comput Biol. 2024;20(11):e1012537. pmid:39527649* View Article
* PubMed/NCBI
* Google Scholar
53. 53.Brodbeck C, Jiao A, Hong LE, Simon JZ. Neural speech restoration at the cocktail party: Auditory cortex recovers masked speech of both attended and ignored speakers. PLoS Biol. 2020;18(10):e3000883. pmid:33091003* View Article
* PubMed/NCBI
* Google Scholar
54. 54.Orf M, Wöstmann M, Hannemann R, Obleser J. Target enhancement but not distractor suppression in auditory neural tracking during continuous speech. iScience. 2023;26(6):106849. pmid:37305701* View Article
* PubMed/NCBI
* Google Scholar
55. 55.Fiedler L, Wöstmann M, Herbst SK, Obleser J. Late cortical tracking of ignored speech facilitates neural selectivity in acoustically challenging conditions. Neuroimage. 2019;186:33–42. pmid:30367953* View Article
* PubMed/NCBI
* Google Scholar
56. 56.Raghavan VS, O’Sullivan J, Bickel S, Mehta AD, Mesgarani N. Distinct neural encoding of glimpsed and masked speech in multitalker situations. PLoS Biol. 2023;21(6):e3002128. pmid:37279203* View Article
* PubMed/NCBI
* Google Scholar
57. 57.Carta S, Aličković E, Zaar J, Valdés AL, Di Liberto GM. Cortical encoding of phonetic onsets of both attended and ignored speech in hearing impaired individuals. PLoS One. 2024;19(11):e0308554. pmid:39576775* View Article
* PubMed/NCBI
* Google Scholar
58. 58.Dimitrijevic A, Smith ML, Kadis DS, Moore DR. Neural indices of listening effort in noisy environments. Sci Rep. 2019;9(1):11278. pmid:31375712* View Article
* PubMed/NCBI
* Google Scholar
59. 59.Seifi Ala T, Graversen C, Wendt D, Alickovic E, Whitmer WM, Lunner T. An exploratory study of EEG alpha oscillation and pupil dilation in hearing-aid users during effortful listening to continuous speech. PLoS One. 2020;15(7):e0235782. pmid:32649733* View Article
* PubMed/NCBI
* Google Scholar
60. 60.Paul BT, Chen J, Le T, Lin V, Dimitrijevic A. Cortical alpha oscillations in cochlear implant users reflect subjective listening effort during speech-in-noise perception. PLoS One. 2021;16(7):e0254162. pmid:34242290* View Article
* PubMed/NCBI
* Google Scholar
61. 61.Fiedler L, Seifi Ala T, Graversen C, Alickovic E, Lunner T, Wendt D. Hearing aid noise reduction lowers the sustained listening effort during continuous speech in noise-a combined pupillometry and EEG study. Ear Hear. 2021;42(6):1590–601. pmid:33950865* View Article
* PubMed/NCBI
* Google Scholar
62. 62.Shahsavari Baboukani P, Graversen C, Alickovic E, Østergaard J. Speech to noise ratio improvement induces nonlinear parietal phase synchrony in hearing aid users. Front Neurosci. 2022;16:932959. pmid:36017182* View Article
* PubMed/NCBI
* Google Scholar
63. 63.Lakatos P, Barczak A, Neymotin SA, McGinnis T, Ross D, Javitt DC, et al. Global dynamics of selective attention and its lapses in primary auditory cortex. Nat Neurosci. 2016;19(12):1707–17. pmid:27618311* View Article
* PubMed/NCBI
* Google Scholar
64. 64.Wöstmann M, Lim S-J, Obleser J. The human neural alpha response to speech is a proxy of attentional control. Cereb Cortex. 2017;27(6):3307–17. pmid:28334352* View Article
* PubMed/NCBI
* Google Scholar
65. 65.Foxe JJ, Snyder AC. The role of alpha-band brain oscillations as a sensory suppression mechanism during selective attention. Front Psychol. 2011;2:154. pmid:21779269* View Article
* PubMed/NCBI
* Google Scholar
66. 66.Herrmann B, Maess B, Henry MJ, Obleser J, Johnsrude IS. Neural signatures of task-related fluctuations in auditory attention and age-related changes. Neuroimage. 2023;268:119883. pmid:36657693* View Article
* PubMed/NCBI
* Google Scholar
67. 67.McCloy DR, Lau BK, Larson E, Pratt KAI, Lee AKC. Pupillometry shows the effort of auditory attention switching. J Acoust Soc Am. 2017;141(4):2440. pmid:28464660* View Article
* PubMed/NCBI
* Google Scholar
68. 68.Tune S, Alavash M, Fiedler L, Obleser J. Neural attentional-filter mechanisms of listening success in middle-aged and older individuals. Nat Commun. 2021;12(1):4533. pmid:34312388* View Article
* PubMed/NCBI
* Google Scholar
69. 69.Decruy L, Lesenfants D, Vanthornhout J, Francart T. Top-down modulation of neural envelope tracking: the interplay with behavioral, self-report and neural measures of listening effort. Eur J Neurosci. 2020;52(5):3375–93. pmid:32306466* View Article
* PubMed/NCBI
* Google Scholar
70. 70.Polyn SM, Norman KA, Kahana MJ. A context maintenance and retrieval model of organizational processes in free recall. Psychol Rev. 2009;116(1):129–56. pmid:19159151* View Article
* PubMed/NCBI
* Google Scholar
71. 71.Baror S, Cohen M, Haik N, Avraham G, Ben-Yakov A. The role of context in continuity and segmentation. Nat Hum Behav. 2026;10(5):988–1005. pmid:41673193* View Article
* PubMed/NCBI
* Google Scholar
72. 72.Sinclair AH, Manalili GM, Brunec IK, Adcock RA, Barense MD. Prediction errors disrupt hippocampal representations and update episodic memories. Proc Natl Acad Sci U S A. 2021;118(51):e2117625118. pmid:34911768* View Article
* PubMed/NCBI
* Google Scholar
73. 73.Caucheteux C, Gramfort A, King J-R. Evidence of a predictive coding hierarchy in the human brain listening to speech. Nat Hum Behav. 2023;7(3):430–41. pmid:36864133* View Article
* PubMed/NCBI
* Google Scholar
74. 74.Mischler G, Li YA, Bickel S, Mehta AD, Mesgarani N. Contextual feature extraction hierarchies converge in large language models and the brain. Nat Mach Intell. 2024;6(12):1467–77.* View Article
* Google Scholar
75. 75.Goldstein A, Zada Z, Buchnik E, Schain M, Price A, Aubrey B, et al. Shared computational principles for language processing in humans and deep language models. Nat Neurosci. 2022;25(3):369–80. pmid:35260860* View Article
* PubMed/NCBI
* Google Scholar
76. 76.LCM Team, Barrault L, Duquenne P-A, Elbayad M, Kozhevnikov A, Alastruey B, et al. Large concept models: Language modeling in a sentence representation space. arXiv. 2024.* View Article
* Google Scholar
77. 77.Tikochinski R, Goldstein A, Meiri Y, Hasson U, Reichart R. Incremental accumulation of linguistic context in artificial and biological neural networks. Nat Commun. 2025;16(1):803. pmid:39824935* View Article
* PubMed/NCBI
* Google Scholar
78. 78.Schiel F. Automatic phonetic transcription of non-prompted speech. In: 1999.
79. 79.Schiel F. A statistical model for predicting pronunciation. 2015.
80. 80.Kisler T, Reichel U, Schiel F. Multilingual processing of speech via web services. Comput Speech Lang. 2017;45:326–47.* View Article
* Google Scholar
81. 81.Boersma P. Praat: doing phonetics by computer. 2007. Available from:http://www.praat.org/* View Article
* Google Scholar
82. 82.Jiang AQ, Sablayrolles A, Mensch A, Bamford C, Chaplot DS, de las Casas D, et al. Mistral 7B. arXiv. 2023.* View Article
* Google Scholar
83. 83.Chalehchaleh A, Winchester MM, Di Liberto GM. Robust assessment of the cortical encoding of word-level expectations using the temporal response function. J Neural Eng. 2025;22(1):10.1088/1741-2552/ada30a. pmid:39719122* View Article
* PubMed/NCBI
* Google Scholar
84. 84.Delorme A, Makeig S. EEGLAB: an open source toolbox for analysis of single-trial EEG dynamics including independent component analysis. J Neurosci Methods. 2004;134(1):9–21. pmid:15102499* View Article
* PubMed/NCBI
* Google Scholar