---
title: Build a Fullstack Stock Portfolio Agent with Mastra and AG-UI - DEV Community
url: https://dev.to/copilotkit/build-a-fullstack-stock-portfolio-agent-with-mastra-and-ag-ui-1ci2
site_name: devto
fetched_at: '2025-08-05T01:05:50.866240'
original_url: https://dev.to/copilotkit/build-a-fullstack-stock-portfolio-agent-with-mastra-and-ag-ui-1ci2
author: Bonnie
date: '2025-07-30'
description: In this guide, you will learn how to integrate a Mastra AI agent with the AG-UI protocol. Also, we... Tagged with webdev, programming, opensource, ai.
...
* Lifecycle events: These events mark the start or end of an agent’s task execution. Lifecycle events includeRUN_STARTEDandRUN_FINISHEDevents.
* Text message events: These events handle streaming agent responses to the frontend. Text message events includeTEXT_MESSAGE_START,TEXT_MESSAGE_CONTENT, andTEXT_MESSAGE_ENDevents.
* Tool call events: These events manage agent’s tool executions. Tool call events includeTOOL_CALL_START,TOOL_CALL_ARGS, andTOOL_CALL_ENDevents.
* State management events: These events keep the frontend and the AI agent state in sync. State management events includeSTATE_SNAPSHOT, andSTATE_DELTAevents.

You can learn more about the AG-UI protocol and its architecturehere on AG-UI docs.

Now that we have learned what the AG-UI protocol is, let us see how to integrate it with Mastra AI agent framework.
...
* Mastra- an open-source TypeScript agent framework designed to give you the primitives you need to build AI applications and features.
...
To get started, clone theOpen AG UI Mastra Demo repositorythat consists of a Node-based backend (agent) and a Next.js frontend (frontend).
...
After that, create a.envfile withOpenAI API KeyAPI key:
...
To test the AG-UI + Mastra AI integration, run the curl command below onhttps://reqbin.com/curl.
...
"
http://localhost:8000/mastra-agent
"
...
Before integrating AG-UI protocol with Mastra AI agents, define and configure your Mastra AI agent workflow as shown in thesrc/mastra/workflows/stock-analysis-workflow.tsfile
...
(
"
The available cash of the user
"
),
...
// Function to emit UI state updates
...
(
"
The investment portfolio of the user
"
),
...
(
"
Available cash after investments
"
),
...
(
"
The total returns from the ticker
"
),
...
(
"
The total returns from the ticker in number
"
),
...
(
"
Percentage of allocation this ticker has
"
),
...
(
"
Current value of ticker in the portfolio
"
),
...
(
"
Percentage of return from this ticker
"
),
...
then
(
gatherStockInformation
)

// Step 2: Fetch historical stock data from Yahoo Finance
...
// Step 3: Calculate portfolio performance and returns
...
// Step 4: Generate market insights using LLM
...
Once you have defined and configured your Mastra AI agent workflow, register the workflow usingworkflowsin the main Mastra instance, as shown in thesrc/mastra/index.tsfile.
...
// Core Mastra framework class for orchestrating agents and workflows
...
// Structured logging library for debugging and monitoring
...
// Database storage provider for telemetry, evaluations, and persistence
...
"
./agents/stock-analysis-agent
"
;

// The intelligent stock analysis agent
...
"
./workflows/stock-analysis-workflow
"
;

// The complete stock analysis workflow
...
// Register the stock analysis workflow for investment calculations
...
// Register the stock analysis agent for handling user conversations
...
// Logger name for identifying log source
...
// Log level - captures info, warn, and error messages (filters out debug/trace)
...
After registering your Mastra AI agent workflow with Mastra instance, create an endpoint with Express that handles frontend requests and responses. Then import the Mastra instance, as shown in thesrc/ag-ui-mastra.tsfile.
...
// Schema for validating incoming agent requests


RunAgentInput
,

// TypeScript interface for agent input data
...
(
"
AG-UI endpoint available at http://localhost:8000/mastra-agent
"
);
...
// Reference the conversation thread
...
In your Mastra AI agent workflow file, configure AG-UI protocol state managementSTATE_DELTAevent in your workflow steps that emits UI state update to show processing status, as shown below.
...
// Event types for UI state updates
...
"
fetch-information-from-user-query
"
,
...
"
Fetches information from user query
"
,
...
"
Fetching information from user query
"
,
...
Then in the Express endpoint, initialize your agent state usingSTATE_SNAPSHOTAG-UI protocol state management event, as shown below.
...
// Initialize empty tool logs array
...
"
render_standard_charts_and_table
"
,
...
// Indicate this is an assistant response
...
Next create a.envfile withOpenAI API KeyAPI key:
...
Navigate tohttp://localhost:3000, and you should see the AG-UI + Mastra AI agent frontend up and running.
...
To create an HttpAgent instance, define it in an API route as shown in thesrc/app/api/copilotkit/route.tsfile.
...
// Register the research agent with the runtime
...
// Use the runtime with our research agent
...
To set up the CopilotKit Provider, theCopilotKitcomponent must wrap the Copilot-aware parts of your application.

For most use cases, it's appropriate to wrap the CopilotKit provider around the entire app, e.g., in yourlayout.tsx, as shown below in thesrc/app/layout.tsxfile.
...
CopilotKit ships with a number of built-in chat components which includeCopilotPopup,CopilotSidebar, andCopilotChat.

To set up a Copilot chat component, define it as shown in thesrc/app/components/prompt-panel.tsxfile.
...
=
"
p-4 border-b border-[#D8D8E5] bg-[#FAFCFA]
"
>
...
=
"
text-lg font-semibold text-[#030507] font-['Roobert']
"
>
...
=
"
inline-block px-2 py-0.5 bg-[#BEC9FF] text-[#030507] text-xs font-semibold uppercase rounded
"
>
...
=
"
mt-3 p-2 bg-[#86ECE4]/10 rounded-lg
"
>
...
=
"
text-xs text-[#575758] font-medium
"
>
...
=
"
text-sm font-semibold text-[#030507] font-['Roobert']
"
>
...
You can learn more about CoAgents’ shared statehereon the CopilotKit docs.

To sync your AG-UI + Mastra AI agent state with the frontend, use theCopilotKit useCoAgent hookto share the AG-UI + Mastra AI agent state with your frontend, as shown in thesrc/app/page.tsxfile.
...
=
"
h-screen bg-[#FAFCFA] flex overflow-hidden
"
>
...
To render the AG-UI + Mastra AI agent's state in the chat UI, you can use theuseCoAgentStateRenderhook, as shown in thesrc/app/page.tsxfile.
...
You can learn more about Human in the Loophereon CopilotKit docs.

To implement Human-in-the-Loop (HITL) in the frontend, you need to use the CopilotKituseCopilotKitActionhook with therenderAndWaitForResponsemethod which allows returning values asynchronously from the render function, as shown in thesrc/app/page.tsxfile.
...
=
"
mt-4 rounded-full px-6 py-2 bg-green-50 text-green-700 border border-green-200 shadow-sm hover:bg-green-100 transition-colors font-semibold text-sm
"
...
=
"
rounded-full px-6 py-2 bg-red-50 text-red-700 border border-red-200 shadow-sm hover:bg-red-100 transition-colors font-semibold text-sm ml-2
"
...
To stream your AG-UI + Mastra AI agent responses or results in the frontend, pass the agent’s state field values to the frontend components, as shown in thesrc/app/page.tsxfile.
...
=
"
w-85 border-r border-[#D8D8E5] bg-white flex-shrink-0
"
>
...
=
"
absolute top-0 left-0 right-0 bg-white border-b border-[#D8D8E5] p-4 z-10
"
>
...
=
"
w-64 border-l border-[#D8D8E5] bg-white flex-shrink-0
"
>
