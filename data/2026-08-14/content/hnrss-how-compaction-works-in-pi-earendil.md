---
title: How Compaction Works in Pi | EARENDIL
url: https://earendil.com/posts/compaction-in-pi/
site_name: hnrss
content_file: hnrss-how-compaction-works-in-pi-earendil
fetched_at: '2026-08-14T11:43:22.707039'
original_url: https://earendil.com/posts/compaction-in-pi/
date: '2026-08-13'
...
If you have ever had a long coding session in a coding agent likePi, Claude Code, or Codex, you will have triggered a compaction.
In this post we explain how compaction works and when Pi needs to compact.

## An LLM conversation

Large language models (LLMs) have limitedcontext windows.
The context window is what the model can "see" while producing a response.
Thetransformer architectureused by LLMs limits how much input they can process.
...
Each request includes a system prompt, loaded files such asAGENTS.md, tool definitions, and the conversation history.
...
The next request then returns an error such asRequest exceeds the maximum size.
...
It might still be a good idea to do, becausethe performance of LLM outputs decrease as the context size grows.
...
Let's look more closely at how Pi specificallyimplements compaction.
...
It can also be manually triggered using the/compactcommand.
...
The number of retained messages varies because Pi uses aconfigurable token budget.
...
It requests"a structured summary of this conversation branch for context when returning later."The prompt specifies sections for goal, progress and key decisions.
...
This keeps the compacted context readable andportable, since we can switch models in Pi and continue using the summary.

## Compaction and prompt caching

Prompt cachingis used by LLM providers to make repeated requests in the same conversation less expensive.
