---
title: 'GitHub - maderix/ANE: Training neural networks on Apple Neural Engine via reverse-engineered private APIs · GitHub'
url: https://github.com/maderix/ANE
site_name: github
content_file: github-github-maderixane-training-neural-networks-on-appl
fetched_at: '2026-07-29T11:47:00.441206'
original_url: https://github.com/maderix/ANE
author: maderix
description: Training neural networks on Apple Neural Engine via reverse-engineered private APIs - maderix/ANE
---
...
This is aresearch project, not a production framework.

The goal was to demonstrate thattraining on the Apple Neural Engine — and potentially other NPUs — is possible, and that the barrier has always been software support, not hardware capability. The ANE is a remarkably capable piece of silicon that Apple restricts to inference-only use through CoreML. This project bypasses that restriction using reverse-engineered private APIs to show what's possible when you give the hardware a chance.

### What This Project Is

* A proof of concept for ANE training via_ANEClientand_ANECompilerprivate APIs
...
* This doesnotreplace GPU training for anything beyond small research models today
...
A from-scratch implementation of transformer training (forward + backward pass) running on the ANE in Apple Silicon. The ANE is a 15.8 TFLOPS FP16 (M4) inference accelerator that Apple does not expose for training. This project reverse-engineers the_ANEClient/_ANECompilerprivate APIs and the MIL (Model Intermediate Language) format to run custom compute graphs — including backpropagation — directly on ANE hardware.
...
Stories110M (12L, dim=768, MHA 12/12)
...
INT8 activations halve L2 SRAM bandwidth between tiles via MILquantize/dequantizeops. Weights useconstexpr_affine_dequantize(int8 stored, fp16 at compile time).
...
GQA models (Qwen3-0.6B) — 10 kernels per layer:Adds separatewoFwd,qBwd,kvBwdkernels for grouped-query attention (Q_DIM ≠ DIM).
...
Seetraining/README.mdfor detailed training instructions.
...
cd
 training/training_dynamic
make MODEL=stories110m 
#
 Stories110M (12L, MHA, 109M params)

make MODEL=qwen3_06b 
#
 Qwen3-0.6B (28L, GQA, 596M params)
...
No external dependencies. Uses only system frameworks + private ANE APIs resolved at runtime viaobjc_msgSend.
...
2. In-memory compilation—_ANEInMemoryModelDescriptorcompiles MIL text + weight blobs directly to ANE programs, no disk mlmodelc needed
...
6. INT8 quantization—constexpr_affine_dequantizefor int8 weights,quantize/dequantizebetween layers for int8 activation caching in L2 SRAM (1.88x throughput)

## Limitations

* SDPA causal masking— ANE hardware ignoresattn_maskin SDPA ops; causal attention is decomposed into separate Q@K^T (ANE) → mask+softmax (CPU) → scores@V (ANE)
* ~119 compile limit— ANE compiler leaks resources; worked around viaexec()restart with checkpoint
...
This project uses Apple's private, undocumented APIs (_ANEClient,_ANECompiler,_ANEInMemoryModelDescriptor). These APIs are not covered by any public stability guarantee and may change or break with any macOS update. This is independent research into Apple Neural Engine architecture, using APIs discovered through runtime introspection for research and educational purposes under fair use and interoperability provisions (seeSega v. Accolade, 1992; DMCA §1201(f)). No Apple proprietary code or binaries are included in this repository. This project is not affiliated with or endorsed by Apple Inc. Use at your own risk.
