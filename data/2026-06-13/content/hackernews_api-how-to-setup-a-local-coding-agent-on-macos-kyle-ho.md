---
title: How to Setup a Local Coding Agent on macOS - Kyle Howells
url: https://ikyle.me/blog/2026/how-to-setup-a-local-coding-agent-on-macos
site_name: hackernews_api
content_file: hackernews_api-how-to-setup-a-local-coding-agent-on-macos-kyle-ho
fetched_at: '2026-06-13T19:33:40.665585'
original_url: https://ikyle.me/blog/2026/how-to-setup-a-local-coding-agent-on-macos
author: Kyle Howells
date: '2026-06-12'
published_date: '2026-06-12'
description: Running Gemma 4 26B-A4B and Qwen3.6 35B-A3B locally with llama.cpp, MTP speculative decoding, multimodal support, and PI as a coding agent.
tags:
- hackernews
- trending
---
...
Gemma 4 now has theMTP draft model available:
...
The first run with MTP came in at 69.2 tokens/second using 4 draft tokens. However, Unsloth's guide onHow to Run MTP Modelsincludes this note:
...
I tested--spec-draft-n-maxvalues from 1 to 6.
...
On my M1 Max machine,3was the fastest, with2close enough that either would be fine. Values above that got slower.

# MLX Comparison

I also tested MLX models throughmlx-lm, to find out which is the faster way to run the model on a Mac, llama.cpp or mlx.
...
I also tried Gemma 4 MTP throughgemma-4-swift-mlx, but the tested 26B 4-bit MLX checkpoints did not match the loader's expected weight keys, and I already had the previous MLX tests, so moved on rather than redownload new models and try to tweak things to match.
...
The llama.cpp server also needs the Gemma 4 multimodal projector in order for the multi-modal part to work (onlythe 12B is natively multi-modal):
...
I used a smallstart_server.shwrapper so it runs inside tmux:
...
"
$ROOT_DIR
/models/unsloth-gemma-4-26B-A4B-it-GGUF/gemma-4-26B-A4B-it-UD-Q4_K_XL.gguf"

DRAFT_MODEL=
"
$ROOT_DIR
/models/unsloth-gemma-4-26B-A4B-it-GGUF/MTP/gemma-4-26B-A4B-it-Q8_0-MTP.gguf"

MMPROJ=
"
$ROOT_DIR
/models/unsloth-gemma-4-26B-A4B-it-GGUF/mmproj-BF16.gguf"

LOG_FILE=
"
$ROOT_DIR
/logs/llama-server-mtp.log"
...
* baseUrlpoints to the llama.cpp OpenAI-compatible server.
...
P.S:Some suggested usingQwen3.6 35B-A3Binstead ofGemma 4 26B-A4B. According to the benchmarks I can find, Qwen is amuchbetter coding agent than Gemma 4.However, it is also slower.Qwen3.6-35B-A3B-UD-Q4_K_XL.gguf+unsloth-Qwen3.6-35B-A3B-MTP-GGUF+mmproj-BF16.ggufresults in 55 tk/s, instead of 72 tk/s. Which is quite significant when you are sitting waiting for it.
...
LLAMA_SERVER=
/Users/kylehowells/Developer/ML-Models/Gemma4/repos/llama.cpp/build/bin/llama-server
