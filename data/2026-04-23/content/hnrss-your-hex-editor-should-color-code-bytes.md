...
site_name: hnrss
content_file: hnrss-your-hex-editor-should-color-code-bytes
fetched_at: '2026-04-23T11:59:28.805622'
original_url: https://simonomi.dev/blog/color-code-your-bytes/
date: '2026-04-21'
published_date: '2026-03-31T11:31:21-05:00'
description: Your hex editor should color-code bytes
tags:
- hackernews
- hnrss
---
...
too often, i see hex editors1that look like this:
...
every time i do, i feel bad for the poor person having to use it (especially if that person is me!). a plain list of bytes makes it hard to notice interesting things in the data. go ahead, try to find the singleC0in these bytes:
...
00000003 33 3C 1E 2C 28 31 C0 1D 11 32 14 05 10 17 3F 01
 3<•,(1×••2••••?•
...
this file starts with themagic bytesKPS, then a bunch of (little-endian) 32-bit integers that range from 0 to 999 (0x3E7). the colors make it quick to recognize that every 32-bit integer is relatively small, as the two high bytes are always00 00. if you look closely, you may notice other patterns, like the numbers counting up every0x18bytes starting at0xC

 

if you're curious about this particular file format,the code that parses itis pretty simple, even if you're not a programmer. there's even awiki pagefor the data it represents, if you're intoFossil Fighters
...
00000150 F4 1A 00 00 30 1B 00 00 6C 1B 00 00 AC 1B 00 00
 ×•
⋄
⋄
0•
...
00000160 F0 1B 00 00 2C 1C 00 00 68 1C 00 00 A8 1C 00 00
 ×•
⋄
⋄
,•
...
00000170 EC 1C 00 00 28 1D 00 00 68 1D 00 00 AC 1D 00 00
 ×•
⋄
⋄
(•
...
00000180 E8 1D 00 00 28 1E 00 00 6C 1E 00 00 A8 1E 00 00
 ×•
⋄
⋄
(•
...
00000190 E8 1E 00 00 2C 1F 00 00 68 1F 00 00 A8 1F 00 00
 ×•
⋄
⋄
,•
...
000001a0 EC 1F 00 00 28 20 00 00 68 20 00 00 AC 20 00 00
 ×•
⋄
⋄
( 
...
000001b0 E8 20 00 00 30 21 00 00 6C 21 00 00 A8 21 00 00
 × 
⋄
⋄
0!
...
000001c0 E4 21 00 00 24 22 00 00 68 22 00 00 A4 22 00 00
 ×!
⋄
⋄
$"
...
000001e0 DC 23 00 00 18 24 00 00 58 24 00 00 9C 24 00 00
 ×#
⋄
⋄
•$
...
000001f0 D8 24 00 00 18 25 00 00 54 25 00 00 94 25 00 00
 ×$
⋄
⋄
•%
...
00000200 D8 25 00 00 14 26 00 00 54 26 00 00 90 26 00 00
 ×%
⋄
⋄
•&
...
this excerpt, starting at0x14, has a long series of increasing 32-bit integers (little-endianagain). each one is an index to a later point in the file, toa structureusually about0x3Cbytes long. the roughly-evenly-spaced indices make for some very pretty rainbow gradients
...
00000050 56 05 E8 43 01 64 52 F5 A4 8D A1 33 D5 98 BF C6
 V•×C•dR××××3××××

00000060 63 EB 4C 8C C6 C3 F8 1A 6A 2A 46 2B C5 F8 15 F3
 c×L××××•j*F+××•×

00000070 60 42 8A 71 E6 56 0C 2A D5 4C 0C 2B 5F 31 A9 18
 `B×q×V•*×L•+_1×•

00000080 4C 8C 55 CC 5B 30 C6 D6 18 37 86 7D BB C3 8F CD
 L×U×[0××•7×}××××

00000090 1E B9 BB BB 91 FA 22 23 9E 71 7A 8B 35 6F F3 84
 •×××××"#×qz×5o××

000000a0 38 DE B7 C9 58 76 A4 9C D7 C5 F8 63 CF A2 B4 BE
 8×××Xv×××××c××××

000000b0 B2 45 BC 8D F7 6A 35 EF E2 B9 CD A7 46 F7 F9 AD
 ×E×××j5×××××F×××

000000c0 7F 6F D7 BC 72 DD DB 9D 6B DE 8F EE C6 35 EF B7
 •o××r×××k××××5××

000000d0 AE 6B E4 9A AE E9 9B 6B AF 23 8E 66 B0 2D 22 47
 ×k×××××k×#×f×-"G
...
this data is compressed using aHuffman code, specifically one compatible withthe Nintendo DS BIOS. it starts with0x20bytes encoding the Huffman tree used, then0x90bytes of compressed bitstream—the actual compressed file contents

 

there's a big difference between the two parts that can be hard to notice without the help of colors. the tree mostly has bytes in the range00–0F(plus some low80s andC0s), but the bitstream has bytes evenly distributed throughout the entire range of00–FF
...
00000038 00 00 00 00 80 80 80 78 46 77 80 08 00 00 00 00
...
00000048 00 00 00 00 88 44 68 12 21 55 46 74 00 00 00 00
...
00000058 00 00 00 70 25 41 33 53 65 13 54 54 08 00 00 00
...
00000068 00 00 70 27 22 13 43 B7 9B 67 54 32 76 08 00 00
...
00000078 00 00 26 22 76 76 98 BA AA BA 59 21 44 75 00 00
...
00000088 00 80 D2 71 99 AA 99 AA A9 AB 99 88 48 43 85 00
 
⋄
××q××××××××HC×
⋄

00000098 00 60 12 A5 A9 9A 99 A9 AA 99 99 CA 48 55 07 00
 
⋄
`•×××××××××HU•
⋄

000000a8 00 38 42 B9 AA 99 9A A9 99 99 89 88 77 78 88 00
 
⋄
8B×××××××××wx×
⋄

000000b8 00 36 86 AA 99 99 B9 AA AA 99 78 78 77 46 75 00
 
⋄
6××××××××xxwFu
⋄

000000c8 80 67 66 A9 99 A9 AA BB BB AA 78 67 57 44 02 08
 ×gf×××××××xgWD••

000000d8 80 23 45 98 A9 AB CB BB BB AA 89 77 57 12 95 00
 ×#E××××××××wW•×
⋄

000000e8 58 2E 55 98 99 BA BB CC BB AB 79 67 56 54 98 00
 X.U×××××××ygVT×
⋄

000000f8 50 52 87 AA A9 BA BB BB CB BB 89 66 56 55 97 00
 PR×××××××××fVU×
⋄

00000108 48 43 A5 AA BA BB CC CC CB 9A 88 66 55 34 84 00
 HC×××××××××fU4×
⋄

00000118 70 44 A8 99 B9 CB CC CC AC 8A 56 45 55 33 05 08
 pD××××××××VEU3••

00000128 00 77 CB A9 AA BC CC CC BC 69 45 43 43 22 A5 08
 
⋄
w×××××××iECC"×•

00000138 80 67 A8 99 BA BB BC CC AB 58 44 33 32 43 A8 00
 ×g×××××××XD32C×
⋄

00000148 00 34 74 A9 AA BB BB BB 7A 45 23 22 23 41 99 08
 
⋄
4t×××××zE#"#A×•

00000158 80 46 74 99 99 AA BA AC 7A 34 22 12 23 41 87 80
 ×Ft×××××z4"•#A××

00000168 00 17 52 99 89 AA AA BB 58 34 23 21 E2 4E A7 09
 
⋄
•R×××××X4#!×N×␣

00000178 00 36 73 99 99 98 98 A9 68 35 22 12 12 4E A9 00
 
⋄
6s×××××h5"••N×
⋄

00000188 70 44 88 87 99 88 78 88 66 45 32 21 E1 62 AA 07
 pD××××x×fE2!×b×•

00000198 70 86 69 65 88 88 68 77 56 44 23 12 21 A7 0A 00
 p×ie××hwVD#•!×⏎
⋄

000001a8 00 90 57 52 85 77 77 66 66 44 33 D1 42 99 00 00
...
000001b8 00 00 70 56 41 55 65 67 54 35 12 21 63 09 00 00
...
000001c8 00 00 00 8A 44 32 22 22 1E 11 12 43 85 80 00 00
...
000001d8 00 00 80 A0 57 55 12 EE 2F 22 32 54 85 08 00 00
...
000001e8 00 00 00 80 99 57 33 45 75 57 66 78 A8 00 00 00
...
this final excerpt is from thebitmap datafor the following image:

 
 

like all the other examples, it comes from the Nintendo DS gameFossil Fighters. specifically, the hole the player makes when digging for fossils:
...
most colorful hex editors have a few categories they sort bytes into, like00bytes, printable ASCII, ASCII whitespace, other ASCII, non-ASCII, orFFbytes
...
i, however, am going further, with 18 total groups: one for each leadingnybble(0X,1X,2X...), plus two extras for00andFF:
...
having more colors makes it possible to recognize more complex patterns, like the ascending offsets fromexample 2or the different sections inexample 3. ASCII text is still recognizable, but instead of solid cyan, it's a variated green and orange:
...
there are a million more examples i could give, like negative numbers intwo's complement(BDFF FF FF), machine code, encrypted data, color palettes, transformation matrices, and so on, but hopefully the ones i've given are enough to get my point across
...
* xxd
* option for byte categories, off2by default
...
if you know any other good ones, please let me know! if you work on any tools that show hexdumps, i highly recommend adding colors, ideally with a large number of groups (feel free to copymine!). at the very least, making00s more subtle than other bytes is extremely helpful
...
while writing this article, i actually started making my own custom hex editor, calledhexapoda>_<. it takes inspiration fromHelixandTeehee(among others), with modal editing, multiple cursors, and selection-first operations (written in Rust, withRatatui!). if enough people want, i might polish it up and write some docs so anyone can use it, but for now, it's just for me :3

1. and also tools likexxdorhexylthat show hex but don't let you edit it⏎
2. by default,xxd's color output is set to "auto", which doesn't output any color for me, so i'm not sure what it's doing⏎

entirely human-made,please don't hesitate toreport a mistakeorsuggest a fix!

discuss onMastodonorLobsters