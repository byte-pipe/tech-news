...
content_file: hackernews_api-github-timescalepg_textsearch-postgresql-extension
fetched_at: '2026-04-02T01:01:15.593567'
original_url: https://github.com/timescale/pg_textsearch
author: tjgreen
date: '2026-04-01'
description: PostgreSQL extension for BM25 relevance-ranked full-text search. Postgres OSS licensed. - timescale/pg_textsearch
tags:
- hackernews
- trending
---
...
🚀Status: v1.0.0 - Production ready. SeeROADMAP.mdfor upcoming features.

## Historical note

The original name of the project was Tapir -TextualAnalysis forPostgresInformationRetrieval. We still use the tapir as our
...
pg_textsearch must be loaded viashared_preload_libraries. Add the following
topostgresql.confand restart the server:
...
Note:<@>returns the negative BM25 score since Postgres only supportsASCorder index scans on operators. Lower scores indicate better matches.
...
Note: Even if EXPLAIN shows a sequential scan,<@>andto_bm25queryalways use the index for corpus statistics (document counts, average length) required for BM25 scoring.

### Filtering with WHERE Clauses

There are two ways filtering interacts with BM25 index scans:

Pre-filteringuses a separate index (B-tree, etc.) to reduce rows before scoring:
...
* Post-filtering tradeoff: The index returns top-k resultsbeforefiltering.
...
This is similar to thefiltering behavior in pgvector,
...
Thebm25querytype represents queries for BM25 scoring with optional index context:
...
Note:The planner requiresmaintenance_work_mem >= 64MBto enable parallel index
...
This is analogous to Lucene'sforceMerge(1). It rewrites all segments into
...
documents up topg_textsearch.default_limit.
...
Number of parallel workers for CREATE INDEX (default 2)

maintenance_work_mem

Memory per worker; must be >= 64MB for parallel builds
...
Max documents scored when no LIMIT clause is present
...
Segments per level before automatic compaction (2-64)

pg_textsearch.bulk_load_threshold

100000

Terms per transaction before auto-spill (0 = disable)

pg_textsearch.memtable_spill_threshold

32000000

Posting entries before auto-spill (0 = disable)

#### Spill thresholds

Thememtable_spill_thresholdcontrols when the in-memory index flushes to
a disk segment. When the memtable reaches this many posting entries, it
automatically spills at transaction commit. Thebulk_load_thresholdtriggers
...
an expression likelower(title) || ' ' || content. As a workaround, use
a generated column:
...
BM25 indexes on partitioned tables usepartition-local statistics. Each
...
This is defined byMAXSTRLENin PostgreSQL's text search implementation.
...
* Elasticsearch: Truncates tokens (configurable viatruncatefilter, default 10 chars)
* Tantivy: Truncates to 255 bytes by default

### PL/pgSQL and Stored Procedures

The implicittext <@> 'query'syntax relies on planner hooks to automatically
detect the BM25 index. These hooks don't run inside PL/pgSQL DO blocks, functions,
or stored procedures.

Inside PL/pgSQL, use explicit index names withto_bm25query():
...
Further language support is available via extensions such aszhparser.
...
bm25_force_merge(index_name) → void

Merge all segments into one (improves query speed)
...
Additional file-writing debug functions (bm25_dump_index(text, text)andbm25_debug_pageviz) are available in debug builds only (compile with-DDEBUG_DUMP_INDEX).
...
wait event names inpg_stat_activitymay be incorrect. Core Postgres tranches
use IDs below 100. If you encounter a conflict, pleaseopen an issue.

## Contributing

SeeCONTRIBUTING.mdfor development setup, code style, and
