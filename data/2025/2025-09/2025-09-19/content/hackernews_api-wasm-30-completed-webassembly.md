...
fetched_at: '2025-09-19T11:06:43.914879'
original_url: https://webassembly.org/news/2025-09-17-wasm-3.0/
author: todsacerdoti
date: '2025-09-17'
description: WASM 3.0 Completed
tags:
- hackernews
- trending
---
...
Published on September 17, 2025 byAndreas Rossberg.

Three years ago,version 2.0of the Wasm
...
lazy. Today, we are happy to announce the release ofWasm 3.0as the new “live” standard.
...
* 64-bit address space.Memories and tables can now be declared to usei64as their address type
instead of justi32. That expands the available address space of Wasm
...
* Multiple memories.Contrary to popular belief, Wasm applications were always able to use multiple
...
into one, to work forallWasm modules. It also paves the way for new uses
of separate address spaces, e.g., for security (separating private data), for
buffering, or for instrumentation.

* Garbage collection.In addition to expanding the capabilities of raw linear memories, Wasm also
...
* Typed references.The GC extension is built upon a substantial extension to the Wasm type
...
newcall_refinstruction.

* Tail calls.Tail calls are a variant of function calls that immediately exit the current
...
* Exception handling.Exceptions provide a way to locally abort execution, and are a common feature
...
* Relaxed vector instructions.Wasm 2.0 added a large set of vector (SIMD) instructions, but due to
...
* Deterministic profile.To make up for the added semantic fuzziness of relaxed vector instructions,
...
* Custom annotation syntax.Finally, the Wasm text format has been enriched with generic syntax for
...
* JS string builtins.JavaScript string values can already be passed to Wasm as externrefs.
...
popping up to target Wasm, such asJava,OCaml,Scala,Kotlin,Scheme, orDart, all of which use the new GC feature.

On top of all these goodies, Wasm 3.0 also is the first version of the standard
that has been produced with the newSpecTectool chain. We
...
stand-alone engines like Wasmtime is on track to completion as well. TheWasm feature statuspage tracks support
across engines.
