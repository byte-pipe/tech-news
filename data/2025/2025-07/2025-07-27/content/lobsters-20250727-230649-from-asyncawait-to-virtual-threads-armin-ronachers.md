...
site_name: lobsters
fetched_at: '2025-07-27T23:06:49.836551'
original_url: https://lucumr.pocoo.org/2025/7/26/virtual-threads/
date: '2025-07-27'
published_date: '2025-07-26T00:00:00'
...
Last November I wrote a post about how the programming interface of threadsbeats the one of async/await. In May,
Mark Shannon brought up the idea ofvirtual threads for
Pythonon Python’s discussion board and also referred back to that article that I
...
that leaks into the programming language to the user and it requirescolored
functions.
...
Unfortunately, recent changes withfree-threadingmake that guarantee rather pointless. Because you still need to write code to
...
important innovation has been the idea ofstructured
concurrency.
...
real threads. For instance, the very popularaiofileslibrary uses a thread
...
multiple tasks, some of which are blocking on a read (withaiofiles) that
...
situation where the presence ofaiofilesin a task group will cause an
...
virtual threads. You can read all about themin the previous
post.
...
this type of programming does not require thinking aboutfutures. Even though it
...
the internal APIs. For instance, the dictionary inresultshas to be locked
for this to work. Likewise, the APIs thatfetch_urlwould use need to support
...
around. One way to get a future is to hold on to the return value of thespawnmethod:
...
This entry was taggedasync,javascript,pythonandthoughts

© Copyright 2025 by Armin Ronacher.Content licensed under the Creative Commons
 attribution-noncommercial-sharealike License.Contact me viamail,bluesky,x, orgithub.You cansponsor me on github.More info:imprint.
 Subscribeto the atom feed.Color scheme:auto,light,dark.
