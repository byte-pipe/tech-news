---
title: Vim Setup
url: https://yobibyte.github.io/vim.html
site_name: lobsters
fetched_at: '2025-07-06T23:06:24.485307'
original_url: https://yobibyte.github.io/vim.html
date: '2025-07-06'
tags: vim
---
...
I love simplicity.
I have a fuckingPhD thesisin simplicity.
...
I have this seamless integration by having a simple custom binding that creates a scratch buffer, runs a command, and sends the output to that scratch buffer:vim.keymap.set("n", "<space>c", function()
...
end)I can even get rid of this binding and just use ":vnew | read !cmd", but I do not want to manually close that buffer.
...
I know a command to track the evolution of a single line in the repo:git log -L 42,42:file.rs.
If I get a merge conflict, I load all the conflicting files into a buffer bygit diff --name-only --diff-filter=U.
...
I run<space>c, and I typerg --vimgrep TODOto get a buffer filled in by the todos.
I can type:cbuferto convert this to a quickfix list for easier navigation with:cnand:cp.
I open my files by typing:efor the files I edit often.
This makes me more aware of the project structure.
If I want to search for a file, I just runrg --files | rg filenameto get it to the buffer.
Then it is a one-line yank and:e PASTE.
I use bookmarks instead of Harpoon, or I type:b fileto move.
If the buffer list is not polluted:b file(you do not have to type the full path or full name), this can be extremely efficient.
...
every time I grep, I see the context around, I can read a couple of lines up or down, I might runtreeinside.
...
I do not have a statusline on, I can pressCtrl-gto get the filename.
