...
fetched_at: '2025-08-12T13:27:06.705664'
original_url: https://dwyer.co.za/static/claude-code-is-all-you-need.html
author: sixhobbits
date: '2025-08-12'
description: Claude Code is all you need
tags:
- hackernews
- trending
---

# Claude CodeIs All YouNeed
...
I cheated a bit because the prompt I used to one-shot this is based a bit on earlier attempts where the model did things that I didn't want, but the app shown below and atsmartsplit.verysmall.siteis the output ofclaude -p "Read the SPEC.md file and implement it"My SPEC.md file is about 500 words (shown in full a bit later).
...
Spoilers: this is what it built. You can visit the startup it built atclaude.dwyer.co.zaor see the GitHub project atgithub.com/sixhobbits/claude-experiments.
...
I had some issues getting it to understand that it wasn't meant to terminate, so I instead told it to write a basic bash script that calls claude with the-pflag and "please continue" whenever it detects its not running.
...
Then I posted its stuff toHacker Newsand Reddit. Luckily I didn't get banned by either for spamming but I did get ignored by both.
...
The project has 100 commits, so if you want to see exactly what it did, you can take a look ateach of those.
...
The first opportunity I had to try Claude Code on something where the stakes were a bit higher was when I got a DM from my friendNicon Slack.
...
ZATechandSboj, two projects I'd taken over and was battling to find the time to give them the attention they deserved.
...
Command not found. Oh yeah, VPS, no shortcuts.export IS_SANDBOX=1 && claude --dangerously-skip-permissionsit is.
...
Last time I solved a similar problem (in 2018)I ended up hacking in Google Docsto create A4 PDFs, but that was more of a templating problem and Google Docs isn't great for layout stuff.

So I builtposters.dwyer.co.za- it lets you generate the background image with AI (I used Claude Code to build everything, but I told it to use GPT for image generation as that's what I'd used before and I think it's better? I don't even know if Anthropic has image generation APIs to be honest and it seemed easier to just use what I knew).
