...
site_name: lobsters
fetched_at: '2025-08-15T23:06:59.029111'
original_url: https://kirancodes.me/posts/log-ocaml-to-lean.html
author: Kiran Gopinathan
date: '2025-08-15'
tags: compilers, ml, plt
---

## Why Lean 4 replaced OCaml as my Primary Language13 Aug, 2025programming_languagescodetheorem_provingperspectives

As I was reading Hacker News today, I happened to stumble upon an
article titled "Why I Chose OCaml as my Primary Language".

This was a particularly interesting read for me: over the
past few years, I have been gradually transitioning1my
primary language away from OCaml to another, newer, and
hotter, language:Lean!2, and while I could understand
...
prolific3former OCaml dev (andchair of the OCaml
workshopfor this year~).
...
I've invested3a considerable amount of effort into
...
obscuring thelogicof your program in the quest for
performance.
...
Who said only Haskell programs could be unreadable4. Anyway,
...
optimisations5, we would be forced to come back to this
...
between concisely expressing myintent(i.e computing this
...
reference-counting with memory reuse algorithmPerceus), and
...
ecosystem of the language itself. OCaml is anamazingly
...
ThePRvery quickly succumbed to orthogonal discussions of
naming and was eventually closed.

Three years later, anotherPRwas created in a second attempt
but also fared the same fate:

It was onlylast year, that this long saga was finally
...
Features such asmodular implicits, an OCaml-specific variant
...
meaning that often updating to the latest version of Leanwillbreak your code, and there's an entire cottage industry
...
mechanism known asppxs, which are syntax-to-syntax level
transformations. Nodes in the AST can be annotated with a tag<node>%<tag_name>, which can then
...
([%ait + 1]transforms tofunit-> it + 1), one might write:
...
syntax tree means that it isextremelydifficult to make any
...
PL research into a well thought out story whereeverything
just works.
...
state of the art PL languages such asRhombus, where the
...
manipulating this tree – for exampleLean.TSyntax `langcorresponds to a generic parse tree
that is constructed from a parser matching thelangsyntax
...
type-directed transformations, Lean also allowselaboration-phasemacros, which operate as terms are being
...
example,attemptingto type-check a term with a specific
...
even use it togenerate LaTeX pdfs.
...
It happened to coincide withphysicallytransitioning my sex
too haha~

2


It's an especially interesting language because it's
both a dependently typed theorem proveranda practical and
...
It's not critical to the main points in this article, but if
you're interested, my credentials to the OCaml community include:

* Author of thefirst type-safe embedded SQLlibrary for OCaml,Petrol
* Developing the firstfederating Activitypub serverin OCaml,OCamlot
* Building the first structural-editing plugin for OCaml,Gopcaml-mode, years before tree-sitter was even conceived, with
several features that still surpass the state of the art in other
comparable tools.
* Authoring state-of-the-artaward winningresearch prototypes with
40,000 LOCs mostly myself in OCaml,Sisysphus.
* Minor brag,correcting a mistakein one of the most widely
referenced texts onOCaml's type-checker.
...
Believe it or not, this was me being a bit
restrained. If we werereallycode-golfing, I might have gone with:
...
The Flambda mode improves things a bit, but still is
extremely conservative in what gets optimised.

6


The parser is so easy to extend that I have some
projects where I've literally embedded the grammar of another
language into Lean itself, such as with myLean bindings to
...
name
:=
 clingo_term_symbolic
)
 clingo_symbol : clingo_term
