---
title: 'Progress Report: Linux 7.0 - Asahi Linux'
url: https://asahilinux.org/2026/04/progress-report-7-0/
site_name: hackernews_api
content_file: hackernews_api-progress-report-linux-70-asahi-linux
fetched_at: '2026-04-26T19:44:41.336452'
original_url: https://asahilinux.org/2026/04/progress-report-7-0/
author: elisaado
date: '2026-04-26'
description: Asahi Linux Progress Linux 7.0
tags:
- hackernews
- trending
---
...
bootstrap it. When you runcurl | sh, you’re actually downloading the boostrap
...
We moved the manifest of installable images into theasahi-installer-data repo,
allowing us to update it independently of the installer codebase.
On top of this, we also now deploy
the installer using GitHub workflows. Going forward, every push to themainbranch ofasahi-installerwill
automatically build the installer and upload it tohttps://alx.sh/dev.
Every tag pushed to GitHub will do the same forhttps://alx.sh.
...
This works by measuring both the brightnessandthe colour characteristics
...
have to caretoomuch about that
...
Partition it creates. A Dracut module then mounts this to a subdirectory of/lib/firmware/, where drivers can find it. However, issues arise when we
...
2. Ensure your distro shipsiio-sensor-proxyas a dependency of your DE (Fedora
Asahi Remix does this)
3. Shut down your Mac
4. Holdthe power button until “Loading startup options” appears on the display
for MacBooks,orfor 5-7 seconds on the desktop machines
5. Select Options from the Apple boot menu
6. Log in with your macOS credentials, then do Command + Shift + T to
open a terminal
7. Invoke the Asahi Installer by runningcurl https://alx.sh | sh
...
not sure exactlywhatit does with this information, but it likely involves
...
with the variant found on the M1 Pro and later SoCs.dd-dreamsis working on enabling PMP support on these machines, so stay tuned!
...
comfortable with making Devicetree changes may defineAPPLE_USE_PMPin their device’s
...
specification for Adaptive Sync,andthe KMS API contract expressly forbid needing
...
to force VRR unconditionally on by setting theappledrm.force_vrrkernel module
parameter once thepull requestis merged.
...
However, that may soon change. The HDMI specification doesnotforbid modesetting
...
either drivers will be expected to enable VRR modes unconditionallyormodesetting between
...
does this. On Linux, this isspeakersafetyd’s
job. No other desktop-class platform does this. This is usually either not done at all,
handled entirely in hardware, or mostly hidden from the OS bySound Open Firmware.
...
sends to a given hardware block and effectively replaying them. But that is notalwaysthe case…
...
the default greeter and session manager, replacing SDDM. This applies tonewinstalls only; users upgrading from previous versions of Fedora Asahi Remix will not
...
our work onOpenCollectiveandGitHub Sponsors.
We would also like to thankBunny, who have supported us with
free CDN and hosting services since the project’s first days.

1. We cannot stress enough just how misleading this name is; the Processor isnotAlways-On↩︎
James Calligeros · 
2026-04-26