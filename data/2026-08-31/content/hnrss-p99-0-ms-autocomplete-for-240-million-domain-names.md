---
title: p99 0 ms* autocomplete for 240 million domain names - Ruurtjan Pul
url: https://ruurtjan.com/articles/p99-0ms-autocomplete-for-240-million-domain-names
site_name: hnrss
content_file: hnrss-p99-0-ms-autocomplete-for-240-million-domain-names
fetched_at: '2026-08-31T17:51:46.202998'
original_url: https://ruurtjan.com/articles/p99-0ms-autocomplete-for-240-million-domain-names
date: '2026-08-31'
description: How Wirewiki's domain-name autocomplete renders suggestions next frame for 99% of keystrokes.
tags:
- hackernews
- hnrss
---
...
I runWirewiki.com, a website to inspect internet infrastructure like domain names. It helps people check (historic) DNS records,
...
The autocomplete is the main way to navigate Wirewiki, so it should be as complete, accurate and fast as possible.
 I want it to beinstant. Like, next frame instant.
...
OnkeyDown(the user starts pressing a key), we prefetch the suggestions for the typed character + any next character. And onkeyUp(the user releases the key), we render the suggestions.
...
That gives us a time budget ofkeyPress1Duration + gap between key presses + keyPress2Duration. If the API returns before the end of the second key press, we'll have the results ready in time.
...
q=wi
 fires the instant 
i
 is pressed; if its response lands before 
k
 is released, completions for 
wik
 render with zero perceived latency.

So for the purpose of this article, we'll define latency askeyUp to results ready for rendering. p99 0 ms means that 99% of the time, the results will be ready before the user even releases the key.
...
Head: in-memory character trie.A trie (prefix tree) stores the top 8 suggestions precomputed for every prefix.
 A prefix lookup is a walk of a few pointers.Worst case time complexity:O(length of what you typed).

Tail: SSD backed memory-mapped block index.The CZDS domains are sorted and delta-compressed into fixed-size blocks with a
 tiny in-memory directory. A lookup binary-searches the directory (27 MB), then linearly scans one block of 256 names. The 240M domain names take about 2.5 GB of disk space. Hot pages are cached in memory by the OS.Worst case time complexity:O(length of what you typed * log(number of domains)).
...
Origin path (nginx + API)

End-to-end (Cloudflare + nginx + API)
...
CDN caching of hot paths andNielsen's 0.1 s "instantaneous" thresholdmake up a lot for this, just not enough to make us hit our target.
...
I would do it if I'd make this into a product. I think this is too niche to build a business on, though. Butemail meif you'd pay for access to this API, I might change my mind.

Oh, and this is the bar I set myself for UX onWirewiki, so if you see anything that could be improved, please let me know as well.