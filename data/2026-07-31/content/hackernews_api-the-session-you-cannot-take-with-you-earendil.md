...
content_file: hackernews_api-the-session-you-cannot-take-with-you-earendil
fetched_at: '2026-07-31T11:44:39.065210'
original_url: https://earendil.com/posts/session-portability/
author: apitman
date: '2026-07-31'
...
That abstraction was never completely true. For instanceprompt caches
liveon somebody else's GPUs, tokenization differs
between models, and sampling is not reproducible (and quite intentionally so).
But thesemantic recordof a session in the form of a transcript could still
...
The naming and marketing around these features can be misleading.encrypted_contentsounds like a privacy feature under the user's control.
...
A better term isprovider-sealed state.

Provider sealing can have a real privacy benefit. OpenAI, for example, can
return encrypted reasoning to a client usingstore: false, then decrypt it in
...
response objects are retained for at least 30 days by default.store: falseis available
and should be used, as it makes it work more like completions: the data is not
stored on OpenAI's servers.

The new Gemini Interactions API has made a similar choice. It defaults tostore: true. On the paid tier interactions are retained for 55 days, and on
...
records the user messages and final text,first.idis now a foreign key into a
...
reasoning can be recovered throughprevious_response_id. Withstore: false,
the API returnsencrypted_content, which the client must preserve and replay.
Persisted reasoning remains opaque even whenreasoning.context: "all_turns"lets a later sample use it.

Anthropic returns the encrypted full thinking in asignaturefield. The
...
These encryption mechanisms permit continuityinsidean ecosystem but they do
...
human-interpretable." The standalone/responses/compactendpoint returns a
...
acompactionblock with a readablecontentfield. It lets the client provide
...
OpenAI's hosted Responses Multi-agent beta returns three new item types:multi_agent_call,multi_agent_call_output, andagent_message. The example
forspawn_agentcontains an encryptedmessageargument, and inter-agent
messages contain onlyencrypted_content. Automatic server-side compaction is
...
forwards it, and the API decrypts it internally for the child. Codex's ownInterAgentCommunication.contentis empty. The exact task is absent from its
...
question ofwhat was that agent asked to do?

Anopen Codex issueasks for the
...
1. The local event log is canonical.Server storage may mirror or accelerate
 ...
 IDs.
2. Storage is explicit.store: falseshould be easy, documented, and
...
distillation. Anthropic'sFebruary 2026
postabout alleged campaigns by DeepSeek, Moonshot, and MiniMax calls them
...
their own walls. OpenAI has also offered an explicitfirst-party API
distillation workflowfor
