...
content_file: hackernews_api-github-xoreaxeaxeaxasm-hall-of-shame-racing-to-the
fetched_at: '2026-08-08T11:25:42.828782'
original_url: https://github.com/xoreaxeaxeax/asm-hall-of-shame
author: piotrgrabowski
date: '2026-08-07'
description: Racing to the bottom of CPU performance. Contribute to xoreaxeaxeax/asm-hall-of-shame development by creating an account on GitHub.
tags:
- hackernews
- trending
---
...
Instruction latency analysis usually focuses on performanceoptimization—making code run as fast as possible. TheAssembly Hall of Shametakes the opposite approach: searching for the absolute floor of
...
Strategy: Usefxrstor64to load 512-byte FPU/MMX/XMM state from a
...
endpoint with non-posted transactions, so CPU 0's 512-bytefxrstor64must
...
A spec-violatingunaligned ymm0 loadthat forced
non-posted dword transactions from stalled GPU registers was used to break the
fundamental design of System Management Mode insmiiiiiiiiiiiiiiii.
...
Strategy:nopdoes nothing. It opens the leaderboard accordingly.
...
Strategy: Regularnopwas too short, but how do we make nothing take
longer? Try alonnnnnng nop.
...
,
 %
rdx
 ; rdx = 2 (high 64 bits: full dividend = 2^65)
...
,
 %
rbx
 ; divisor → quotient = 2^65/5 ≈ 7.4×10^18
...
$
31
 ; 0 bytes allocated, nesting depth 31 (maximum)
...
rax
 ...
 ; rax -> dirty cache line resident in L3
...
Strategy: Saturate all write-combining line-fill buffers withmovntistores to distinct cache lines, forcingmfenceto drain the full LFB write
...
rdi
 ...
 ; ×16 distinct cache lines — saturate the write-combining LFBs
...
mfence
 ; must drain all pending LFB writes before retiring
...
fldl subnorm
 ; 1e-310: value < DBL_MIN, biased exponent = 0

faddl subnorm
 ; source is subnormal → FP microcode assist
...
Strategy: Alignlock-prefixed operand to straddle cache-line
...
Strategy: Useproject:nightshyftto identify high latency MSRs. MCG_CTL on Zen look like a winner: may be a
...
Strategy: Useproject:nightshyftto identify high latency model-specific-registers: VIA uses an undocumented
...
Strategy: Usemmioticto identify
...
accesses, whichstillisn't technically allowed but works anyway.
...
Strategy: Search MMIO space for slowest registers in PCIe fabric, hit
unknown GPU register, use 32-byteunalignedMMIO read to get nine dword
register accesses, which is evenlessallowed than the aligned version, but
works anyway.
...
Strategy: Extendfxrstor64 (baseline)by starving the fabric while the
load is in flight — a fleet of hammer cores pounds a different high-latency
MMIO register with tight 4-byte reads, saturating the PCIe root complex and
...
from fxrstor64:xsavestate area is 8KB vs 512 bytes, 16x size ->
...
rsi
 ...
 ; rsi -> MMIO region, same technique as fxrstor64
