---
title: 'GitHub - jiji262/douyin-downloader: A practical Douyin downloader for both single-item and profile batch downloads, with progress display, retries, SQLite deduplication, and browser fallback support. 抖音批量下载工具，去水印，支持视频、图集、合集、音乐(原声)。 · GitHub'
url: https://github.com/jiji262/douyin-downloader
site_name: github
content_file: github-github-jiji262douyin-downloader-a-practical-douyin
fetched_at: '2026-09-13T14:49:42.338218'
original_url: https://github.com/jiji262/douyin-downloader
author: jiji262
description: A practical Douyin downloader for both single-item and profile batch downloads, with progress display, retries, SQLite deduplication, and browser fallback support. 抖音批量下载工具，去水印，支持视频、图集、合集、音乐(原声)。 - jiji262/douyin-downloader
---
...
Beta:The desktop app is currently in closed beta. To try it, download the build from theReleasespage.

Douyin link download

TikTok download workspace

YouTube workbench

Paste a video, gallery, profile, or collection link and start in one click.

Download public videos, photo posts, and profiles without signing in.

Scan videos, Shorts, channels, and playlists, then configure video, MP3, or subtitle downloads.
...
Sync creators, filter new works, add notes, and download directly from the list.

Browse collected videos, series, and liked works from the current Douyin account.

Track job results, retry failures, and open output folders.

Screenshots were captured from the current desktopmainbuild. Demonstration data is used for privacy.
...
/music/{music_id}
 (prefers direct audio, fallback to first related aweme)
...
Logged-in favorites collections

/user/self?showTab=favorite_collection
 + 
mode: [collect, collectmix]

No-watermark preferred

Automatically selects watermark-free video source

Highest-quality selection

Auto-picks highest bitrate from 
video.bit_rate
 ladder (video + live-photo)

Live stream recording

live.douyin.com/{room_id}
 → FLV/HLS, preserves partial data on stream end

Comments collection

Per-aweme comments (+ optional replies) saved as 
*_comments.json
...
Bark / Telegram / Webhook on download completion

Extra assets

Cover, music, avatar, JSON metadata

Video transcription

Optional, using OpenAI Transcriptions API
...
Records download metadata; does not decide incremental skips

Incremental downloads

Disk-based skip/redownload via 
...
Launches browser when pagination is blocked, manual CAPTCHA supported

Download integrity check

Content-Length validation, auto-cleanup of incomplete files

Progress display

Rich progress bars, supports 
progress.quiet_logs
 quiet mode
...
* Browser fallback is fully validated forpost;like/mix/musiccurrently relies on API pagination
* number.allmix/increase.allmixare retained as compatibility aliases and normalized tomix
* collect/collectmixcurrently work for the account represented by the logged-in cookies only
* collect/collectmixmust be used alone and cannot be combined withpost/like/mix/music
* increasecurrently applies topost/like/mix/music; favorites collection modes do not support incremental stop
...
/config.yml:/app/config.yml -v 
$(
pwd
)
/Downloaded:/app/Downloaded douyin-downloader
...
Run as REST API server (requires 
...
#
 0 = record until broadcaster ends
...
The recorder saves an FLV file underDownloaded/{author}/live/plus a*_room.jsonmetadata snapshot. If the broadcaster ends the stream, network goes idle, or you
Ctrl+C, any already-recorded bytes are preserved (the.tmpfile is promoted to
the final file).
...
false 
#
 true will fetch each comment's second-level replies (extra API calls)
...
Generates a{date}_{title}_{aweme_id}_comments.jsonnext to the media file.
...
Finished jobs are pruned by TTL (default 24h) and max-jobs (default 500) — in-flight jobs are never pruned. Configure viaserver.max_jobs/server.job_ttl_seconds.
...
webhook 
#
 works with 企业微信/飞书/钉钉 bot URLs too
...
Withtrue, the downloader skips an item only when its non-empty primary media
...
Set a mode tofalseto redownload and atomically replace existing files within the
...
Current behavior applies tovideo items only(image-note items do not generate transcripts).
...
#
 empty: same folder as video; non-empty: mirrored to target dir
...
Ifdatabase: true, job status is also recorded in SQLite tabletranscript_job(success/failed/skipped).
...
Plainpytestis also supported now:
...
/
music
; logged-in favorites mode additionally supports standalone 
collect
/
collectmix
...
true
: skip existing primary media on disk; 
false
: redownload and overwrite current scope
...
Optional HTTP/HTTPS proxy setting
...
Default withfolderstyle: trueanddatabase_path: dy_downloader.db:
...
The program uses adatabase record + local filedual check to decide whether to skip already-downloaded content. To force re-download, you need to clean up accordingly:
...
rm -rf Downloaded/AuthorName/post/
*
_
<
aweme_id
...
sqlite3 dy_downloader.db 
"
DELETE FROM aweme WHERE aweme_id = '<aweme_id>';
"
...
sqlite3 dy_downloader.db 
"
DELETE FROM aweme WHERE author_name = 'AuthorName';
"
...
By default,progress.quiet_logs: truesuppresses logs during progress stage.Use--show-warningsor-vtemporarily when debugging.
...
* whethertranscript.enabledistrue
* whether downloaded items are videos (image-notes are not transcribed)
* whetherOPENAI_API_KEY(ortranscript.api_key) is valid
* whetherresponse_formatsincludestxtorjson

### 5) How to view download history?

sqlite3 dy_downloader.db 
"
SELECT aweme_id, title, author_name, datetime(download_time, 'unixepoch', 'localtime') FROM aweme ORDER BY download_time DESC LIMIT 20;
"
