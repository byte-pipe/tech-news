---
title: A quick look at zero-knowledge proofs | Max Bernstein
url: https://bernsteinbear.com/blog/zkp/
site_name: tldr
content_file: tldr-a-quick-look-at-zero-knowledge-proofs-max-bernstei
fetched_at: '2026-08-17T19:26:04.931241'
original_url: https://bernsteinbear.com/blog/zkp/
author: Max Bernstein
date: '2026-08-17'
published_date: '2026-08-07T00:00:00+00:00'
description: 'NB: This isn’t about crypto. I don’t care about crypto.'
tags:
- tldr
---
...
Chrismessaged me the other week asking
...
Chris and I went around in circles for a bit until we decided to take a look atone of the original papers(PDF) by Goldreich, Micali, and Widgerson. We only really read page 23 (labeled
...
common inputA graphG(V, E)(n = |V|,m = |E|).

The following four steps are executedm²times, each time using independent
coin tosses.
...
corresponding vertex. More specifically, the prover chooses a permutationπ ∈R S₃, placesπ(φ(i))in a box markedi(∀i ∈ V), locks all boxes
and sends them (without the keys) to the verifier.

(V1) The verifier chooses at random an edgee ∈R Eand sends it to the
prover. (Intuitively, the verifier asks to examine the colors of the endpoints
ofe ∈ E.)

(P2) Ife = (u, v) ∈ E, then the prover reveals the colors ofuandvby sending the verifier the keys to boxesuandv. Otherwise, the
prover does nothing.

(V2) The verifier opens boxesuandvusing the keys received and
...
verifierrejectsand stops. Otherwise, the verifier continues to the next
iteration.

If the verifier has completed allm²iterations then itaccepts.

We’ll come back to the number of iterations. For now let’s try to just dooneiteration. For each step, I’ll annotate the code with “Only prover” or “Only
...
If you are building alongside the blog post, I recommend usingrandom.seed(0)so your randomness doesn’t change between runs of your program. I also
recommend setting the environment variablePYTHONHASHSEEDto0if you are
usinghashfor the same stability reasons.
...
might be better to use a cryptographic hash function likehashlib.sha256:
...
To get around this, we can add what’s called anonceto each node and its
...
generator for your nonces. You should consider something likesecrets.token_hex()from thesecretsmodule (Python 3.6+). Maybe even
consider using thehmacmodule.

Finally, we can send thehashed_coloringto the verifier and begin step V1.
...
satisfies the 3-color conditions. It sends off a request for an arbitrary edgee:
...
box locking protocoleach time we reveal an edge(also once per round), so
...
hash function (and the same hash seed if usinghash).
...
If either of these two conditions doesn’t check out, the verifierrejects.
...
accept (i.e., complete all them²rounds without detecting that “something is
wrong”) is bounded above by(1 - m⁻¹)^(m²)” (wherem = |E|). Which is
...
you’re at 0.0045% possibility of “cheating”. Atm² = 1,000,000rounds, it’svery low.
...
permuted colors). You can visitthe docs
pageto see the API docs and build a
client yourself!
...
to verify that someone has a valid 3-coloring of a graph without learninganyinformation about the 3-coloring. So what? Is there anything else we can prove
