---
title: 'GitHub - yikart/AiToEarn: Let''s use AI to Earn! · GitHub'
url: https://github.com/yikart/AiToEarn
site_name: github
content_file: github-github-yikartaitoearn-lets-use-ai-to-earn-github
fetched_at: '2026-05-11T13:46:42.968776'
original_url: https://github.com/yikart/AiToEarn
author: yikart
...
AiToEarn 通过AI 自动化，帮助 OPC（一人公司）、创作者、品牌与企业在全球主流平台上构建、分发并变现内容。
...
💡方式 ②③④ 都需要先获取 API Key，请先看如何获取 API Key。
...
1. 打开aitoearn.cn（中国用户）或aitoearn.ai（国际用户），注册并登录
2. 点击左侧菜单设置
3. 在API Key中点击创建，复制生成的 Key
...
* 中国版：使用aitoearn.cn获取的 API Key
* 国际版：使用aitoearn.ai获取的 API Key
...
找到并编辑claude_desktop_config.json，添加：
...
💡 如果你是自部署用户，将aitoearn.ai替换为你自己的地址（如localhost:8080）。
...
在docker-compose.yml的aitoearn-server服务中添加（API Key 获取方式见上方说明）：

请根据RELAY_API_KEY来源选择RELAY_SERVER_URL：中国版 Key 使用https://aitoearn.cn/api，国际版 Key 使用https://aitoearn.ai/api。环境和 Key 不匹配会导致 401。
...
📖 完整部署指南（生产环境配置、AI 服务、OAuth、存储等）请参阅DOCKER_DEPLOYMENT_CN.md。
...
如果你在使用过程中遇到困难、使用问题或异常情况，优先通过GitHub Issues提交反馈，方便我们统一跟进和处理。
