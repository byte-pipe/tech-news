...
site_name: tldr
content_file: tldr-aws-strands-harness-an-ai-agent-that-runs-any-mode
fetched_at: '2026-09-21T16:50:10.953789'
original_url: https://thelettertwo.com/2026/09/21/aws-strands-harness
author: Ken Yeung
date: '2026-09-21'
published_date: '2026-09-21T16:00:00+00:00'
description: AWS released Strands Harness, a ready-to-run AI agent with web search, shell, memory, and file editing built in. You supply the model—and can swap it anytime.
tags:
- tldr
---
...
Amazon Web Services (AWS) has a new tool for developers. On Monday, the companyreleased Strands Harness, a ready-to-run agent that can search the web, run commands, edit files, remember what it did yesterday, and hand off work to helper agents. The only thing that’s missing is the intelligence to direct it. That’s the part developers choose and can change whenever they want.

Strands Harness supports the latest models from Amazon Bedrock, Anthropic, OpenAI, and Google. Alternatively, for on-machine use, developers can elect to use a local Ollama model.

Today’s announcement extends Strands Agents, AWS’s open-source SDKintroduced in May 2025. It allowed developers to quickly build and run agents with a few lines of code. The launch of the Strands Harness further accelerates that work, giving builders pretty much everything they’d need in an agent, with the exception of the brains.
...
“Getting an agent running and getting it to perform well at a reasonable cost are different engineering problems,”Marc Brooker, AWS vice president and distinguished engineer, toldThe AI Economyin an email. “An SDK gives you the building blocks, but you still need to decide how to manage context, integrate tools, and tune the system prompt, then evaluate whether those choices improve performance or just consume more tokens.”
...
Brooker said the Strands Harness was designed for three workload families: operations and DevOps automation, back-office process automation, and conversational experiences inside products. “The common thread is long-running, multi-step tasks that you’d otherwise script or queue for a person,” he said. “The agent can write and run code as part of a task, but it’s not a coding assistant. That’s a different category withgreat products already in it.”
...
But the company isn’t the first to provide such a framework to developers. Others with similar tools include Google’sAntigravity agentand Microsoft’sAgent Framework Harness. It’s also not AWS’s first harness; it has one withAgentCore. Even so, these agents are built to eliminate setup time and configuration issues. With something out of the box, developers can focus on prototyping what their agents can do, spending time and resources ensuring they provide real value for their team and organization.
...
### HubSpot Puts Breeze Assistant in Charge of Its AI Agents

HubSpot knows workers use multiple AI agents, but they don’t have time to manage them or even figure out which one best suits the job. So, to make things easier, the company this week decided to put its newly upgraded Breeze Assistant in charge, empowering it to coordinate other agents on the HubSpot platform. In […]
...
### Live Blog: Dreamforce 2026—the Agentic Enterprise Gets Real
...
### AWS Makes Quick’s Desktop App Generally Available

Amazon Web Services has released a new way for enterprise customers to use its Quick AI assistant. On Thursday, the company announced that a desktop version is now generally available, offering another way to use it beyond the browser. This app also gives the AI more context by pulling in work-related information stored on the […]