---
title: 'GitHub - Andyyyy64/whichllm: Find the local LLM that actually runs and performs best on your hardware. Ranked by real, recency-aware benchmarks, not parameter count. One command, run it instantly. · GitHub'
url: https://github.com/Andyyyy64/whichllm
site_name: github
content_file: github-github-andyyyy64whichllm-find-the-local-llm-that-a
fetched_at: '2026-06-08T12:33:15.033211'
original_url: https://github.com/Andyyyy64/whichllm
author: Andyyyy64
description: Find the local LLM that actually runs and performs best on your hardware. Ranked by real, recency-aware benchmarks, not parameter count. One command, run it instantly. - Andyyyy64/whichllm
---
...
After install, runwhichllmdirectly. For one-off runs, replacewhichllmwithuvx whichllm@latest.
...
The 32B modelfits your card fine— whichllm still ranks the 27B #1,
...
ranked onactiveparams, quality ontotal.)

## What can I run?

Real top picks (snapshot 2026-05 — your results trackliveHuggingFace
...
Fitting a model into your VRAM is the easy part. The hard part is knowingwhich of the models that fit is actually the best— and that is what
...
trusted.
* Evidence-graded and guarded— Every score is taggeddirect/variant/base/interpolated/self-reportedand
...
* One command, scriptable—whichllmprints the answer; add--json | jqfor pipelines. No TUI, no keybindings to memorize.
...
* One-command chat—whichllm rundownloads and starts a chat session instantly
* Code snippets—whichllm snippetprints ready-to-run Python for any model
...
creates an isolated environment viauv, installs dependencies, downloads the
...
Works withall model formats:

* GGUF— viallama-cpp-python(lightweight, fast)
* AWQ / GPTQ— viatransformers+autoawq/auto-gptq
* FP16 / BF16— viatransformers

Get acopy-paste Python snippetinstead:
...
#
 allow id/base-model matches

whichllm --evidence strict 
#
 id-exact only (same as --direct)
...
whichllm run 
#
 auto-pick best for your hardware
...
JSON model rows includeestimated_tok_per_sec,speed_confidence,speed_range_tok_per_sec, andspeed_notes. The speed range is a planning
...
step is usually needed beforeollama run.
...
Merged LiveBench / Artificial Analysis / Aider / Vision / Arena ELO / Open LLM Leaderboard, weighted by source confidence
...
log2
-scaled world-knowledge proxy (MoE uses total params)

Quantization

× penalty

Lower-bit quants discounted multiplicatively

Evidence confidence

×0.55–1.0

none / self-reported ×0.55, inherited ×0.78, direct full

Runtime fit

×0.50–1.0

partial-offload ×0.72, CPU-only ×0.50

Speed

-8 to +8

Usability gate vs a fit-dependent tok/s floor; reported with confidence and range metadata

Source trust

-5 to +5

Official-org bonus, known-repackager penalty

Popularity

tie-breaker

Downloads/likes; weight shrinks as evidence strengthens
...
1. Model fetching— Fetches popular models from HuggingFace API:* Text-generation (downloads + recently updated)
* GGUF-filtered (separate query for coverage)
* Vision models (image-text-to-text) when--profile visionorany
...
3. Benchmark evidence— Five resolution levels, increasingly discounted:* direct— Exact model ID match
...
* self_reported— Uploader-claimed eval (heavily discounted)Inheritance is rejected when a model's params diverge more than 2× from
its family's dominant member, catching draft / MTP / abliterated forks
that share afamily_idwith a much larger base.
4. Cache—~/.cache/whichllm/:* models.json— 6h TTL
...
know what it picked for your rig. Drop it inIssues.
...
* Python 3.11+
* NVIDIA GPU detection vianvidia-ml-py(included by default)
