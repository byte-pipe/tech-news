---
title: goliath32.com - The Zilog Z80 has turned 50
url: https://goliath32.com/blog/z80.html
site_name: hnrss
content_file: hnrss-goliath32com-the-zilog-z80-has-turned-50
fetched_at: '2026-07-18T11:24:55.351706'
original_url: https://goliath32.com/blog/z80.html
date: '2026-07-17'
description: The Zilog Z80 has turned 50
tags:
- hackernews
- hnrss
---
...
As of writing, the Zilog Z80 processor was officially launched 50 years
 ...
 on the moon,decadescloser to WWII than to the present day,
...
The Z80 itself also spawned many clones and derived architectures over
 ...
 variants like the pipelined and higher clockedeZ80, mainly
...
The whole thing sure taught me a number of interesting lessons about
 ...
 assembler, writing a compiler is something you canactually do).
...
Once upon a time, theComputer Terminal Corporation (CTC)built a new, programmable terminal, theDatapoint
...
Neither company finished their design in time. When Intel had the chip
 ready, originally named1201based on a systematic naming
...
While TI ultimately canned their design, Intel went ahead and successfully
 commercialized their version as the8008(like the4004,
...
The 8008 has 7 registers:A,B,C,D,E,H,L. WhereAis the designated
 accumulator, the others can be used as operands or scratch. As the name
 might imply,HandLtogether form theHigh andLow part of a memory pointer. Accessing memory is done through
 an 8th pseudo registerM, representing the memory byte thatHLpoints to.
...
The program counterPCis pretty much never visible directly. There
 are dedicated function call & return instructions, but the processor
 uses aninternalreturn address stack that is 8 levels deep. The
...
From there, it gets a bit tricky. The CPU does not have a general purpose
 stack that it can safe registers to, all memory access needsHL,
 but you don't want to clobberHLin the interrupt handler. The
...
Federico Faggin,
 who was brought over from the4004project, was pushing to start
...
Even before the project was finally approved, Federico Faggin got
 approval to hireMasatoshi Shimaaway fromBusicomto
...
The stack pointer can be exchanged or moved in to/out ofHL,
 registers can be pushed on or popped of the stack pairwise.
 BesidesHL, the other register pairs areBC,DE,
 andAF(accumulator and ALU flags), but the 8080 assembly prefers
 to call the later the "program status word"PSW.

Memory addresses are bumped up to full 16 bits, giving the machine a 64k
 address space. The I/O ports are bumped up to 256.BCandDEnow also allow rudimentary indirection (loading/storing the
 accumulator), the accumulator andHLcan be loaded/stored at an
...
A few double-byte arithmetic operations are added that can work on
 ...
 arithmetic and 16 bit counters. Using those instructions onAFwould actually act onSPinstead.
...
A few things that are noteworthy here: instructions that act on register
 pairs always use a single register as a mnemonic forboth, the "X"
...
Furthermore, the double byte arithmetic has no influence on the ALU flags,
 ...
 decrementingBC, we need to manually check if both registers are
 zero.
...
Intel addressed at leastsomeof those short comings with
 ...
 needingsomespecialized support chips.
...
Inspired by the 6800, it adds two index registersIXandIYthat can be used in place ofHL(same encoding but with an opcode
 prefix) and with an immediate offset.

TheAF,BC,DEandHLregister pairs are bank
...
The Z80 also adds a bunch of bit rotate, bit test & set instruction
 ...
 (usingBCas a counter), self-repeating block transfer, block
...
PUSH
 BC 
; full name of the register pair
...
JP
 NZ, loop 
; overloaded name, condition is an argument
...
The Z80 only needs a single 5V supply and a single clock signal. Many
 ...
 exposed by the chip, such as asMREQorIORQto indicate
 memory or I/O access,RD/WRsignals that indicate exactly
 what the name suggests, and anM1signal that indicates the current
...
Zilog eventually split off of Exxon again in 1989 and went public in 1991,
 ...
 equity and actual electronics companies, currently owned byLittelfuse.
