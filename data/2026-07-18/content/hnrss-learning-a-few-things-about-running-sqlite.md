...
site_name: hnrss
content_file: hnrss-learning-a-few-things-about-running-sqlite
fetched_at: '2026-07-18T11:24:54.293562'
...
as the database.
When I was getting started with using SQLite as database for a website I read abunchof blog posts about how it is totally fine to use SQLite in production for a
small site and I think itistotally fine, but what I did not fully appreciate
...
### ANALYZEis apparently important

Today I was running a query (usingSQLite’s FTS5for
full-text search) on a table with 4000 rows and it took 5 seconds. That seemed
wrong to me: computers are fast!

It turned out that what I needed to do was to runANALYZE!
...
but my best guess is that it was some sort ofaccidentally quadraticthing.

ANALYZEgenerates “statistics” (I guess about the number of rows in each table? and presumably other things?)
...
in my database that I don’t want to be there (for example completed tasks fromdjango-tasks-db), and I want to clean them up.
...
than theANALYZEthing. The database is pretty small (maybe 10000 rows?) and
...
I setretention: 400hin my config file in an attempt to
...
My current project only has one database, but one trick I used withMess with DNSwas to split the tables into three separate
...
been great, I think the move from Postgres was a great choice for that project.
...
for the first time in 2022 and I only learned thatANALYZEexisted today!
