---
title: 'GitHub - xodn348/han: A compiled programming language with Korean keywords, written in Rust · GitHub'
url: https://github.com/xodn348/han
site_name: hackernews_api
content_file: hackernews_api-github-xodn348han-a-compiled-programming-language
fetched_at: '2026-03-15T19:14:07.173040'
...
* REPL— interactive mode withhgl repl
* LSP server—hgl lspfor editor hover docs and completion
* Static typing— 5 primitive types:정수(int),실수(float),문자열(string),불(bool),없음(void)
* Arrays—[1, 2, 3], indexing, negative indexing,.추가/.삭제/.정렬/.역순etc.
...
* Format strings—형식("이름: {0}", 이름)positional or형식("이름: {이름}")named
...
* Rust(1.70+)
* clang (forhgl build/hgl run) —xcode-select --installorbrew install llvm
...
Then open theeditors/vscodefolder in VS Code and pressF5to launch with syntax highlighting + LSP support.
...
* Enums —열거 방향 { 위, 아래 }, access with방향::위
...
* 반복for-loop with init, condition, step
* 반복 x 안에서 배열for-in loop — iterates arrays, strings, ranges
* 동안while-loop
* 멈춰(break),계속(continue)
* 맞춰pattern matching — integer, string, bool, wildcard_, binding
* Range operator —0..10creates[0, 1, 2, ..., 9]
...
* Impl block methods with자신(self):구현 사람 { 함수 인사(자신: 사람) { ... } }
...
* 가져오기 "파일.hgl"— runs another.hglfile in the current scope
...
hgl build
 with closures/methods

Codegen stubs — interpreter only

hgl build
 with string/array methods
...
Han brings this elegance into programming. When you write함수 피보나치(n: 정수) -> 정수, you are not just defining a function — you are writing in a script that was purpose-built for clarity and beauty.
...
Han uses more tokens, not fewer.Korean keywords average 2-3 tokens each vs 1 for English. This is because BPE (Byte Pair Encoding) tokenizers are trained on English-dominant data —functionappears billions of times and merges into a single token, while함수is rare and gets split into byte-level pieces.

This is a tokenizer training bias, not a property of Korean. If BPE were trained on Korean-heavy data,함수could easily be a single token.
...
Loop control—멈춰(break) and계속(continue):
