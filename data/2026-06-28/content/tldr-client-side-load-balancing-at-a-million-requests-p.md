...
site_name: tldr
content_file: tldr-client-side-load-balancing-at-a-million-requests-p
fetched_at: '2026-06-28T19:34:40.168203'
original_url: https://engineering.zalando.com/posts/2026/06/client-side-load-balancing.html
date: '2026-06-28'
published_date: '2026-06-23T00:00:00+02:00'
description: How we built an in-process client-side load balancer for a million requests per second of internal fan-out traffic, what we layered on top (N-ring fade-in, occupancy-based bounded load, and...
tags:
- tldr
---
...
deploy
's churn into a single ring update
...
The fix came from queueing theory.Little's Lawstates that average concurrency equals arrival rate multiplied by average service time:\(L = \lambda W\). Instead of counting request arrivals, we accumulate request duration in a sliding time window and divide by the window length:
...
In-flight
0
sampled between bursts, nothing is open
Throughput
1,000
requests per second, regardless of duration
Occupancy
~1.0
1,000 x 1 ms = 1.0 s of work per second
...
We then weight that signal by latency, an idea borrowed fromFinagle: a pod's cost is its concurrency multiplied by its latency relative to the cluster average.
...
B
's misses drag its own to 60 ms
...
HeretotalLoadis what this client process observes across all pods, its own local view rather than a cluster-wide figure. A local pod's expected load is the sum of both terms, because it draws traffic from both rings. As the local weight ramps from 1% to its ceiling during fade-in, the threshold tracks it, so bounded load only fires on a genuine outlier rather than on the asymmetry that zone affinity creates on purpose.
...
We're hiring! Do you like working in an ever evolving organization such as Zalando? Consider joining our teams as aBackend Engineer!
...
How we contributed two features to Debezium to solve WAL growth and enable safer logical replication for our event...Read more...
