...
site_name: hnrss
content_file: hnrss-choosing-a-public-dns-resolver
fetched_at: '2026-06-28T11:37:07.891995'
original_url: https://evilbit.de/dns-resolver-guide.html
date: '2026-06-27'
description: Choosing a Public DNS Resolver
tags:
- hackernews
- hnrss
---
...
Maximum privacy and no logging
Minimal or no query logging, privacy-first operator

Block malware and phishing
Security blocklist on by default or via a simple variant

Block ads and trackers
Network-wide ad and tracker filtering

Parental controls and adult-content blocking
Family or adult-content filter available

No filtering (unaltered DNS)
Returns answers exactly as published

Fully customizable filtering
Choose your own blocklists or rules via an account

Top-tier speed (global anycast)
Large low-latency anycast network

Non-commercial operator
Nonprofit, registry, community or public-interest, not a for-profit company
...
Must validate DNSSEC

Must offer IPv6
Provides IPv6 resolver addresses
...
Nonprofit, community or public-interest
...
Filtering (default and variants)
...
Operational and community data
...
* ICANNIdentifier Technology Health Indicators (ITHI): tracks theshare of resolvers that perform DNSSEC validationandresolver concentration(how few resolvers serve most queries).
* DNS-OARCworkshop talksandpast-event archive: semi-annual operator and researcher presentations on encrypted DNS, resolver performance, and measurement.
* APNIC Labsmeasurements(the live data behind many OARC and ICANN talks, by Geoff Huston):DNSSEC validation rate by country,use of public resolvers, andDoH and DoT adoption.

Smaller, community-run, and regional resolvers

Niche, hobby, community, or country-specific services that are not in the comparison above. Worth knowing about, but check their current status and policies before relying on them. The European entries are catalogued byEuropean Alternatives. Resolvers based in heavily censored or sanctioned regions may enforce local content rules or, conversely, exist mainly to bypass geo-blocks, so treat those with extra care.
...
* DigitalcourageandArtikel10(Germany): privacy-focused non-profit resolvers with DoH and DoT (German-language sites).
...
* Shecan, Electro, Begzar, and403.online(Iran): widely used inside Iran mainly to reach developer and cloud services that block Iranian IP addresses; special-purpose, with limited published policy.
