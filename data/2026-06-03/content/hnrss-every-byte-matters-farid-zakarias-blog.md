---
title: Every byte matters | Farid Zakaria’s Blog
url: https://fzakaria.com/2026/06/01/every-byte-matters
site_name: hnrss
content_file: hnrss-every-byte-matters-farid-zakarias-blog
fetched_at: '2026-06-03T20:09:15.682955'
original_url: https://fzakaria.com/2026/06/01/every-byte-matters
date: '2026-06-03'
published_date: '2026-06-01T13:07:00-07:00'
...
The cost of each new field is rarely considered. Performance is often considered from aclassic computer scienceperspective by considering asymptotic analysis of the algorithms and data structures in-use.

Turns out that even within a growth scale for your algorithm, such as a simple for-loopO(N), time can vary dramatically if we have a little deeper understanding of the underlying hardware.

First, let’s understand our current machine. Let’s take a peek at ourcache lineandpagesizes.
...
Theinstancesnumber is a reflection of how the caches are shared amongst CPUs. If I had 10 CPUs, each one has their ownL1dcache, whereas two of them would share anL2cache.

Our cache line size is64 bytes.
...
When you reada singlebyte from memory, the hardware will fill the surrounding 64 bytes into the cache line.
...
The sizes for each cache, is the number returned bylscpudivided by the number of cores or instances; i.e. 352 KiB ÷ 10 instances = ~35 KiB.
...
Let’s consider an example where we want to iterate over a single structMonsterand pull out theboolean is_aliveto filter them.
...
Each cache line would fill with a single monster, and we would fetch only theis_alivebyte.
...
We can observe up to30ximprovements when the Monster struct is 1KiB 🤯
...
This means thetotal size of your collectiondetermines your performance tier.
...
Rather than graph it logarithmically, which I find sometimes is easy to miss, I have included a zoomed in graph. We can see that all struct sizes hit the samestaircaselike pattern as they go through the various cache levels however the larger struct sizes areshifted left, meaning they hit the increase earlier.
