...
content_file: hackernews_api-how-llms-actually-work-0xkato
fetched_at: '2026-06-06T11:34:31.972266'
original_url: https://www.0xkato.xyz/how-llms-actually-work/
author: 0xkato
date: '2026-06-03'
published_date: '2026-06-01T13:37:00+00:00'
...
Tiny explainer: token IDA token ID is the integer the model uses for one vocabulary entry. The model works with the number, not the written word itself.
...
Tiny explainer: vocabularyThe vocabulary is the tokenizer’s fixed list of pieces. Each piece has an ID, and the model can only directly receive IDs from that list.
...
A token ID like1024is just a row index. It doesn’t mean anything by itself. The thing that gives it meaning is a giant table called the embedding matrix.
...
Tiny explainer: vectorA vector is a list of numbers. In a transformer, each token becomes a vector so the model can do math with it.
...
Tiny explainer: embedding matrixThe embedding matrix is a lookup table. Token ID in, learned vector out.
...
You can do arithmetic on embeddings and it sometimes works. The famous example isking − man + woman ≈ queen. The geometry of embedding space carries real semantic structure, even though nobody told the model to build it that way.
...
Tiny explainer: positional encodingPositional encoding is how the model gets order information. It tells the model where each token sits in the sequence.
...
Tiny explainer: RoPERoPE stands for Rotary Position Embeddings. Instead of adding a position vector, it rotates token vectors so relative distance shows up during attention.
...
Tiny explainer: Q, K, VQuery means “what am I looking for,” Key means “what do I match with,” and Value is the information that gets copied when the match is strong.
...
Tiny explainer: dot productA dot product is a simple way to score how aligned two vectors are. Higher alignment means a stronger match.
...
Tiny explainer: softmaxSoftmax turns raw scores into weights that add up to 1. Big scores get big weights, small scores get small weights.
...
Tiny explainer: causal maskingCausal masking hides future tokens. It keeps a decoder-only language model from looking ahead while predicting the next token.
...
Tiny explainer: induction headAn induction head is an attention head that notices repeated patterns in the prompt and helps continue them.
...
Tiny explainer: attention headAn attention head is one independent attention pass with its own learned projections.
...
Tiny explainer: KV cacheThe KV cache stores old Key and Value vectors during generation. It saves the model from recomputing the whole prompt every time it adds a token.
...
Tiny explainer: GQAGrouped-Query Attention lets multiple query heads share fewer key/value heads. That cuts KV-cache memory while keeping many query views.
...
Tiny explainer: non-linearityA non-linearity is a function that prevents the network from collapsing into one big linear transformation.
...
Tiny explainer: MoEMixture of Experts means the model has several feed-forward networks and routes each token through only a few of them.
...
Tiny explainer: residual connectionA residual connection adds a block’s output back to the vector it started from. It gives information and gradients a shortcut through the network.
...
Tiny explainer: layer normalizationLayer normalization rescales a token vector so its numbers stay in a stable range while the model trains.
...
Tiny explainer: RMSNormRMSNorm is a cheaper normalization method that rescales vector size without subtracting the mean first.
...
Tiny explainer: logitsLogits are raw scores for each possible next token. They become probabilities only after softmax.
...
Tiny explainer: temperatureTemperature controls randomness during sampling. Low temperature makes the model more conservative; high temperature makes it more varied.
...
Tiny explainer: speculative decodingSpeculative decoding uses a small draft model to guess ahead, then asks the larger model to verify several guessed tokens at once.
...
Tiny explainer: weightsWeights are the learned numbers inside the model. Training changes those numbers until the model predicts text well.
...
Feedback is extremely welcome. If any of this interests you, please reach out onX. I love making new friends.