---
title: The world in which IPv6 was a good design - apenwarr
url: https://apenwarr.ca/log/20170810
site_name: hnrss
content_file: hnrss-the-world-in-which-ipv6-was-a-good-design-apenwarr
fetched_at: '2026-04-19T19:41:08.055681'
original_url: https://apenwarr.ca/log/20170810
date: '2026-04-19'
description: Last November I went to an IETF meeting for the first time. The IETF is an interesting place; it seems to be about 1/3 maintenance grunt wo...
tags:
- hackernews
- hnrss
---
...
because I wanted to see how people would react toTCP BBR, which was being
...
it be better if it had just beenIPv4 with more address bits? But it's not,
...
your phone connection was literally just a very long wire ("OSI layer1"). A
...
wrote the first Linux arcnet driverand arcnet poetryway
...
pretty small, so it was onlykindof a pain.
...
details right now, but basically the problem iscongestion
control. The deep dark secret of ethernet bridging is that it assumes
...
you had to choosewhichrouter on the ethernet bus is supposed to
pick it up. We can't just use the IP destination field for that, because
we're already using that for thefinaldestination, not the router
...
networks. Ethernet is not actually a bus anymore. It justpretendsto be a bus. Basically, we couldn't get ethernet's famousCSMA/CDto keep working as speeds increased, so we went back to the good old star
...
No problem! 802.11 has a thing called 3-address mode. They add athirdethernet MAC address to every frame, so they can talk about the real
...
the IPv6 teamforgot to fix. Unfortunately, while they were
...
transport (layer 4). If, instead, we had identified sessions usingonlylayer 4 data, then mobile IP would have worked perfectly.
...
1Edit 2017-08-16:It turns out that nothing in this
section requires IPv6. It would work fine with IPv4 and NAT, even roaming
across multiple NATs.

2Edit 2017-08-15:Some people asked what "some care to
...
3Edit 2017-10-24:Besides QUIC, there are several other
candidates for such a protocol, includingMinimaLT. I didn't mention
...
Update 2020-07-09:I've postedmore thoughts on IPv4/IPv6 migration
and interoperabilityon the Tailscale blog.
