...
class BaseScraper:
    ...
    def __init__(self, test_mode=False, test_output_dir=None):
        ...
        self.logger.info(f"Initialized scraper for site: {self.site_name}")
        if self.test_mode:
            self.logger.info(f"Running in TEST MODE with output directory: {self.test_output_dir}")

    def _get_page(self, url):
        try:
            self.logger.info(f"Fetching URL: {url}")
            response = self.session.get(url, timeout=30)  # Add 30-second timeout
    ...
    def _parse_html(self, html, selector):
        ...
        try:
            ...
            self.logger.info(f"Found {len(items)} items matching selector: {selector}")
            return items
        except Exception as e:
            self.logger.error(f"Error parsing HTML with selector {selector}: {str(e)}")
            return []
        ...
        try:
            ...
            if self.test_mode and self.test_output_dir:
                ...
                try:
                    ...
                    folder_path = organized_dirs["raw"]
    ...
    def extract_description(self, element, selectors=None, max_length=150) -> str:
