...
class StructuredLogger:
    ...
    def timer(self, operation: str, log_start: bool = True):
        ...
        try:
            ...
            self.info(f"Completed {operation}", operation=operation, status="completed", duration_seconds=round(elapsed, 3))
        except Exception as e:
            elapsed = time.time() - start_time
            self.error(f"Failed {operation}", operation=operation, status="failed", duration_seconds=round(elapsed, 3), error=str(e), error_type=type(e).__name__)
            raise

    def log_metric(self, metric_name: str, value: Any, **tags):
        """Log a metric with optional tags."""
        self.info(f"Metric: {metric_name}", metric_name=metric_name, metric_value=value, metric_type="gauge", **tags)

    def log_scraper_result(self, scraper_name: str, success: bool, items_count: int = 0, duration: float = 0, error: Optional[str] = None):
        """Log scraper execution results."""
        self.info(f"Scraper {scraper_name} {'succeeded' if success else 'failed'}", scraper=scraper_name, success=success, items_count=items_count, duration_seconds=round(duration, 3), error=error)

    def log_http_request(self, method: str, url: str, status_code: Optional[int] = None, duration: Optional[float] = None, error: Optional[str] = None):
        """Log HTTP request details."""
        self.info(f"HTTP {method} {url}", http_method=method, http_url=url, http_status_code=status_code, duration_seconds=round(duration, 3) if duration else None, error=error)
...
class MetricsCollector:
    ...
    def record_scraper_run(self, scraper_name: str, success: bool, items_count: int = 0, duration: float = 0, error: Optional[str] = None):
        """Record metrics for a scraper run."""
        if scraper_name not in self.metrics["scrapers"]:
            self.metrics["scrapers"][scraper_name] = {"runs": 0, "success": 0, "failed": 0, "items_total": 0, "total_duration": 0, "errors": []}
        ...
        if success:
            ...
            if error:
                ...
                self.metrics["errors"].append({"scraper": scraper_name, "error": error, "timestamp": datetime.utcnow().isoformat() + "Z"})

        scraper_metrics["total_duration"] += duration
        self.metrics["performance"]["total_duration"] += duration

    def record_http_request(self, status_code: Optional[int] = None, success: bool = True, duration: float = 0):
    ...
    def record_operation_duration(self, operation: str, duration: float):
        ...
        if operation not in self.metrics["performance"]["by_operation"]:
            self.metrics["performance"]["by_operation"][operation] = {"count": 0, "total_duration": 0, "min_duration": float("inf"), "max_duration": 0}
    ...
    def get_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "scrapers": {"total_runs": total_runs, "success_rate": round(success_rate, 2), "by_scraper": self.metrics["scrapers"]},
