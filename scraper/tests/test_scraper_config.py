...
class TestScraperConfig(unittest.TestCase):
    ...
    def setUp(self):
        ...
        (self.project_root / "scraper" / "logs").mkdir(parents=True)
        (self.project_root / "pyproject.toml").touch()

        # Reset global config
        import scraper.core.config

        scraper.core.config._global_config = None
    ...
    def test_explicit_project_root(self):
        ...
        # Also resolve data and logs dir paths for consistent comparison
        self.assertEqual(config.data_dir.resolve(), (self.project_root / "data").resolve())
        self.assertEqual(config.logs_dir.resolve(), (self.project_root / "scraper" / "logs").resolve())
    ...
    def test_environment_variable_fallback(self):
        ...
        with patch.dict(os.environ, {"SCRAPER_PROJECT_ROOT": test_root}):
            ...
            with patch("scraper.core.config.__file__", "/tmp/fake_location.py"):
...
class TestScraperConfigIntegration(unittest.TestCase):
    ...
    def test_real_project_detection(self):
        ...
        try:
            ...
            self.assertTrue((config.project_root / "scraper").exists())
        except RuntimeError:
