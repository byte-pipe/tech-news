...
class TestGitHubScraper(unittest.TestCase):
    ...
    @patch("scrapers.github.BaseScraper._get_page")
    @patch("scrapers.github.BaseScraper._parse_html")
    def test_scrape_with_period(self, mock_parse, mock_get_page):
        ...
        self.assertEqual(len(result), 1)  # Should get one result (deduplicated)
