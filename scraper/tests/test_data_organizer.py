...
from scraper.core.data_organizer import DataOrganizer  # noqa: E402
...
class TestDataOrganizer(unittest.TestCase):
    ...
    def test_create_organized_structure(self):
        ...
        # Test with default date (only creates essential directories)
        self.organizer.create_organized_structure()

        # Verify only essential directories were created
        paths = self.organizer.get_organized_paths()
        essential_dirs = ["base", "raw"]
        for path_type in essential_dirs:
            self.assertTrue(paths[path_type].exists())
            self.assertTrue(paths[path_type].is_dir())

        # Test creating all directories explicitly
        self.organizer.create_organized_structure(directories=["base", "raw", "content", "summaries", "summaries_local", "exports"])
    ...
    def test_ensure_directory(self):
        """Test ensure_directory creates specific directories on-demand."""
        test_date = "2025-06-02"

        # Test creating specific directory types
        content_dir = self.organizer.ensure_directory(test_date, "content")
        self.assertTrue(content_dir.exists())
        self.assertTrue(content_dir.is_dir())

        summaries_dir = self.organizer.ensure_directory(test_date, "summaries")
        self.assertTrue(summaries_dir.exists())
        self.assertTrue(summaries_dir.is_dir())

        # Test invalid directory type
        with self.assertRaises(ValueError):
            self.organizer.ensure_directory(test_date, "invalid_type")
