...
def fetch_content_for_results(site_name, results, max_articles=5, force=False):
    ...
    data_organizer = DataOrganizer(str(config.project_root))
    now = datetime.now()
    date_folder = now.strftime("%Y-%m-%d")

    # Create organized directory structure for this date
    try:
        data_organizer.create_organized_structure(date_folder)
        organized_dirs = data_organizer.get_organized_paths(date_folder)
        logger.info(f"Using organized content directory: {organized_dirs['content']}")
    except Exception as e:
        ...
        try:
            ...
            organized_dirs = {"content": fallback_content_dir}
    ...
    results_to_fetch = results[:max_articles] if max_articles > 0 else results
    ...
    with structured_logger.timer("sequential_scraper_execution"):
        ...
        for scraper_class in scrapers:
            ...
            try:
                ...
                if scraper_class != scrapers[-1]:  # Don't delay after last scraper
                    time.sleep(2)
